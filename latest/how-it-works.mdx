...
<Steps>
  ...
  <Step title="Issue is picked up and marked In Progress">
    ...
    - **Recent** — even after a job finishes, the same issue won't be re-dispatched while a dedup entry from its previous dispatch is still on record.

    A separate condition also stops a dispatch: an issue the dispatch breaker has [parked](/latest/reference/troubleshooting#an-eligible-issue-stopped-being-dispatched) after repeated failed dispatches is skipped until an operator unparks it.
  ...
  <Step title="The pull request is reviewed again">
    ...
    - Anything else means the pull request still needs someone to look at it, and [Post-push review](/latest/reference/post-push-review) covers each of those cases.
  </Step>
  ...
  <Step title="Late review feedback handling">
    <Warning>
      Like the previous step, this runs on the orchestrator's GitHub webhook and has no polling fallback.
    </Warning>
