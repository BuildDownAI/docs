...
<Steps>
  ...
  <Step title="Planning phase (if enabled)">
    ...
    Implementation starts automatically as soon as planning succeeds.

    <Note>
      A manual-approval gate before implementation is planned but not yet built — the `autoApprovePlans` mapping field is reserved for it and currently has no effect.
    </Note>
  ...
  <Step title="Issue closes out automatically when the PR merges">
    Once your team merges the PR, AI-Implement marks the originating issue complete on its own — nothing to configure.
    ...
    <Tabs>
      ...
      <Tab title="Jira">
        `AI-Implement Status` is set to `Merged`.
      </Tab>
    </Tabs>

    This is checked on every poll cycle, so it lands within 60 seconds even without a webhook; a configured GitHub webhook makes it near-instant. It also runs for paused project mappings, so pausing new work never leaves already-merged issues stuck open.
  </Step>
</Steps>
...
<Note>
  If you label a parent issue with `AI-Implement` and it has children also labeled `AI-Implement` (Linear only), AI-Implement changes how it dispatches that tree: each child's PR merges into a shared long-running branch for the parent instead of your base branch, and the parent's own work waits until all of its children finish. When the whole tree completes, that shared branch is merged up automatically (or opened as a PR against your base branch for you to review, at the top level). Everyday single-issue runs are unaffected.
</Note>
...
| *(Done state)* | `Merged` | PR has merged; issue is complete |
