...
<Steps>
  ...
  <Step title="Issue is picked up and marked In Progress">
    ...
    - **In-flight** — if a job is already actively running for this issue, no new dispatch happens.
    - **Already dispatched** — once an issue has been dispatched, it stays out of consideration indefinitely, not for a fixed window. Only a failed or timed-out run, the merge-reconciliation loop reaching a terminal issue, or clearing it by hand in the admin UI's Audit log makes it eligible again.
  ...
  <Step title="Planning phase (if enabled)">
    ...
    Claude reads the codebase in read-only mode, guided by your repo's `PLANNING.md` template. It then posts exactly three structured comments to the issue:

    - **Implementation Map** — approach, a Create/Modify/Test/Delete file list, and constraints or hazards the implementer must not break. Cross-story coordination notes fold in here when the issue has dependencies or sibling stories.
    - **Acceptance Bar** — a numbered list of falsifiable, diff-checkable claims the reviewer can check off.
    - **Risks & Open Questions** — edge cases, unknowns, and potential problems found while exploring the codebase.
  ...
  <Step title="Automated review runs on the PR">
    After the PR is opened, an automated review pass runs against the diff. If it finds no blocking issues, it posts "✅ Approved" and stops.

    If it finds blocking issues, it posts "⚠️ Reviewer found issues — starting fix pass", then Claude fixes them and force-pushes a revision before the review runs again — up to three passes by default. If blockers remain after the cap, the PR is left open with the remaining issues listed for a human.

    This pass runs on the same model as implementation by default — the `model:` value in `WORKFLOW.md`, or `claude-sonnet-4-6` when that's unset.
  </Step>
  ...
  <Step title="Issue is updated to Ready for Review">
    ...
    Your team can now review the PR and its review comments in GitHub, and see the status change in your ticketing system — no context-switching required.
  </Step>

  <Step title="Gap-fill runs with /ai-implement">
    After reviewing the PR and its review comments, you can send Claude back in for another pass without starting a new run. Comment `/ai-implement` on the PR (in any PR comment).

    On a repo synced with the current workflow templates, the orchestrator's own webhook picks up the comment directly — this needs [`GITHUB_WEBHOOK_SECRET`](/latest/configuration/environment-variables#param-github-webhook-secret) configured. A repo that hasn't synced since this changed still carries the older `comment-trigger.yml` workflow, which picks up the comment instead without needing a webhook.

    Claude checks out the existing branch, reads the same `WORKFLOW.md` template, and focuses on the gaps identified in the previous run. It pushes new commits to the same branch, and the review comment is updated automatically.
