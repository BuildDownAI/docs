...
<Steps>
  ...
  <Step title="Issue is picked up and marked In Progress">
    ...
    - **Recent** — even after a job finishes, the same issue won't be re-dispatched while a dedup entry from its previous dispatch is still on record. That entry has no timer; it stays until something clears it.
