...
<Tabs>
  <Tab title="Orchestrator runtime">
    ...
    The admin UI supports sign-in via SSO (Google and/or Microsoft) as well as a shared access code. The UI is enabled when **either** at least one SSO provider **or** `ADMIN_ACCESS_CODE` is configured, and returns `404` on all `/admin` and `/api/*` routes when neither is set.

    <ParamField body="ADMIN_ACCESS_CODE" type="string" deprecated>
      Shared password for the admin UI at `/admin`. Prefer SSO below for anything beyond local development — this fallback still works (with a timing-safe comparison) but logs a deprecation warning on every use.
    </ParamField>

    <ParamField body="OAUTH_REDIRECT_BASE_URL" type="string">
      Base URL that SSO providers redirect back to after sign-in (e.g. `https://your-orchestrator.fly.dev`; `http://localhost:8080` locally). Required for either SSO provider to work — without it, sign-in fails with "OAuth is not configured."
    </ParamField>

    <ParamField body="GOOGLE_OAUTH_CLIENT_ID" type="string">
      Google OAuth client ID. Set together with `GOOGLE_OAUTH_CLIENT_SECRET` to show the Google sign-in button on the admin login screen.
    </ParamField>

    <ParamField body="GOOGLE_OAUTH_CLIENT_SECRET" type="string">
      Google OAuth client secret, paired with `GOOGLE_OAUTH_CLIENT_ID`.
    </ParamField>

    <ParamField body="MICROSOFT_OAUTH_CLIENT_ID" type="string">
      Microsoft (Entra) OAuth client ID. Set together with `MICROSOFT_OAUTH_CLIENT_SECRET` and `MICROSOFT_OAUTH_TENANT` to show the Microsoft sign-in button.
    </ParamField>

    <ParamField body="MICROSOFT_OAUTH_CLIENT_SECRET" type="string">
      Microsoft OAuth client secret, paired with `MICROSOFT_OAUTH_CLIENT_ID`.
    </ParamField>

    <ParamField body="MICROSOFT_OAUTH_TENANT" type="string">
      The specific Microsoft tenant (directory) ID to sign in against.

      Must be a single-tenant GUID. A multi-tenant value (`common`, `organizations`, or `consumers`) disables Microsoft sign-in at startup, since the tenant can't be safely verified in that mode.
    </ParamField>

    <ParamField body="OAUTH_ALLOWED_DOMAINS" type="string">
      Comma-separated email domains allowed to sign in via SSO.

      A signed-in identity must match this list or `OAUTH_ALLOWED_EMAILS` to be admitted. Leaving both unset locks SSO sign-in out for everyone, even with a provider configured.
    </ParamField>

    <ParamField body="OAUTH_ALLOWED_EMAILS" type="string">
      Comma-separated individual email addresses allowed to sign in via SSO, in addition to `OAUTH_ALLOWED_DOMAINS`.
    </ParamField>
    ...
    <ParamField body="GITHUB_WEBHOOK_SECRET" type="string">
      ...
      Leaving it unset disables the webhook endpoint entirely, which turns off two capabilities:
      - **Automatic follow-up runs** when a reviewer leaves blocking feedback on a pull request. Nothing else polls for review feedback.
      - **The `/ai-implement` comment trigger.** Commenting `/ai-implement` on a pull request is handled by this same webhook — without it, comments go unanswered.
    ...
    <AccordionGroup>
      ...
      <Accordion title="Gap-fill trigger">
        <ParamField body="GAP_FILL_TRIGGER_SECRET" type="string">
          ...
          This is unrelated to the built-in `/ai-implement` comment trigger — that path is handled by the GitHub webhook (see `GITHUB_WEBHOOK_SECRET` above), not this endpoint. Set `GAP_FILL_TRIGGER_SECRET` only if you're driving gap-fill runs from your own external automation.
        </ParamField>
      </Accordion>
  ...
  <Tab title="Target-repo workflow secrets">
    ...
    **No ticketing provider secrets belong here.** The orchestrator now applies every Linear and Jira update itself, so the synced workflows never read a ticketing credential. 
    ...
    <Warning>
      These variables only take effect on repos that still have `comment-trigger.yml` installed — repos that haven't re-run **Sync workflows** since it was replaced by the orchestrator's own comment handling. Once a repo re-syncs, `/ai-implement` comment-triggered gap-fill runs automatically inherit the same per-project caps configured in the admin UI; see [Project mappings](/latest/configuration/team-repo-mappings).
    </Warning>

    Three repository variables cap `/ai-implement` comment-triggered gap-fill runs on repos still using `comment-trigger.yml`, the same way per-project caps cap orchestrator-initiated runs. All three are optional — unset means "use the default."
    ...
    <ParamField body="AI_IMPLEMENT_SKILLS_REPO" type="string" deprecated>
      Skills repository as `owner/repo`, cloned into the workspace so Claude can use its skills during `/ai-implement` comment-triggered runs — only on repos still using `comment-trigger.yml` (see the Gap-fill run caps note above).

      Once a repo re-syncs onto the current workflow templates, comment-triggered runs pick up the project's skills repository from the admin UI mapping automatically, and this variable has no effect.
    </ParamField>
