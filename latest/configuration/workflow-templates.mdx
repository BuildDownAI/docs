...
### The `/ai-implement` comment trigger

Commenting `/ai-implement` on an open pull request dispatches `claude-implement.yml` with the existing PR number, so Claude can address remaining gaps without opening a new branch. This is handled by the orchestrator itself (via a GitHub webhook), not by a workflow file synced into the target repo — see [`GITHUB_WEBHOOK_SECRET`](/latest/configuration/environment-variables#param-github-webhook-secret) for what needs to be configured.

<Note>
  Repos that predate this change may still have an old `comment-trigger.yml` workflow file. It's no longer used, and a sync removes it if present.
</Note>
