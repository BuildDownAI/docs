...
### `comment-trigger.yml` (removed by sync)

Older setups had this file listen for `/ai-implement` comments on open pull requests and dispatch `claude-implement.yml` directly. Sync now **deletes** it if present — the `/ai-implement` comment trigger is handled by the orchestrator's GitHub webhook instead, so re-syncing an older repo also migrates it off this file. See [Configure webhook delivery](/latest/setup/github-app#create-the-github-app) for the webhook setup that replaces it.
...
A command-line fallback is available if you're scripting against a self-hosted orchestrator:
...
<Note>
  This fallback is not fully equivalent to an orchestrator-driven sync: it still syncs `comment-trigger.yml` rather than removing it. Prefer the admin UI's **Sync workflows** action or saving the project mapping, which migrates the repo off `comment-trigger.yml` onto the webhook-based trigger.
</Note>
...
<Warning>
  Never put `${PLANNING_CONTEXT}` in the body yourself. The runner substitutes it automatically and also appends the same planning context after your template. Referencing it manually makes it appear twice in the prompt.
</Warning>
...
<ParamField body="gap_analysis_model" type="string">
  **Reserved — not currently read.** The seeded `WORKFLOW.md` ships this key commented out, and the front matter parser accepts it, but no step consumes it.
...
After stripping front matter and HTML comments, the runner substitutes a fixed set of `${UPPER_SNAKE}` variables into the body of `WORKFLOW.md`. This isn't a call to the `envsubst` utility, which would pull from the whole environment — only the variables listed below are replaced.

Any other `${TOKEN}`-shaped text is silently replaced with an empty string, so a shell example containing one will read as blank. A plain `$VAR` without braces is left untouched.

You can use these variables anywhere in the template:
...
| `${PLANNING_CONTEXT}` | The fetched planning comments. Already substituted into the prompt automatically — see the warning above before referencing it directly |
