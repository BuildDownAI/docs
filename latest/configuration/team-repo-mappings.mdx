...
<ParamField body="dependencyTokenScope" type="string">
  Grants the run a short-lived, read-only token so its install step can fetch private sibling repositories that the GitHub App installation can see. Labeled **Dependency Token Scope** in the project's Edit dialog.

  The only accepted value is `installation`. Leave it unset to disable the token.

  <Warning>
    This is all-or-nothing: the token can read every repository the App installation covers, not a chosen subset. It also requires a publicly reachable orchestrator, since the token is fetched over the runner callback.

    A run dispatched without that reachability simply proceeds without private-dependency access, rather than failing.
  </Warning>
</ParamField>
...
<Note>
  Both fields only take effect on a repo synced to the current workflow templates. A repo still running the older, pre-sync `comment-trigger.yml` workflow doesn't send these patterns to the runner, so they're silently ignored — run **Sync workflows** on the project's row to move it onto the current templates.
</Note>
...
<ParamField body="autoMerge" type="boolean" default="false">
  Reserved for per-project control over child-pull-request auto-merging. It does **not** currently affect dispatch behavior: the orchestrator merges this project's child pull requests into their shared grouping branch once checks pass regardless of this setting, for any non-paused project with a parent/child group.
