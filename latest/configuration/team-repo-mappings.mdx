...
<ParamField body="skillsRepo" type="string">
  ...
  Leave unset to run without project skills. For `/ai-implement` comment-triggered runs, set the matching repo variable instead — see [`AI_IMPLEMENT_SKILLS_REPO`](/latest/configuration/environment-variables#param-ai-implement-skills-repo).
</ParamField>

<ParamField body="sensitiveAddPatterns" type="array">
  Extra glob patterns treated as sensitive for this project, on top of the built-in list. A run that stages a matching file is blocked before it commits — see [Blocked by a security guardrail](/latest/reference/error-codes#blocked-by-a-security-guardrail).
...
<ParamField body="dependencyTokenScope" type="string" default="off">
  When set to `installation`, the run fetches a short-lived, read-only GitHub credential before dependency install so package managers can pull from private sibling repositories the same GitHub App installation can see. Leave unset and dependency installation only reaches what the target repo's own token can read.

  <Warning>
    The scope is all-or-nothing — it reads every repository the GitHub App installation covers, not a chosen subset. It also needs a publicly reachable orchestrator, since the token is fetched over the runner callback; a run dispatched without a progress token skips the fetch and proceeds without private-dependency access rather than failing.
  </Warning>

  <Note>
    On Fly Machines deployments, the primary GitHub token only reads the target repository — it no longer covers the whole installation. If your deployment relied on that broader access to reach private sibling repos, set this field to `installation` to restore it.
  </Note>
</ParamField>
...
<ParamField body="planningEnabled" type="boolean" default="true">
  ...
  See [AI planning phase](/latest/configuration/planning) for details.
</ParamField>
...
<ParamField body="autoApprovePlans" type="boolean" default="true">
  ...
  See [AI planning phase](/latest/configuration/planning) for what actually happens between the two phases.
</ParamField>
...
<ParamField body="autoMerge" type="boolean" default="false">
  ...
  Applies only to issues that belong to a parent/child group — see [Feature-branch grouping](/latest/reference/feature-branch-grouping). Ordinary single-issue runs are unaffected.
...
<Tip>
  These three caps also exist as repo variables — `AI_IMPLEMENT_MAX_TURNS`, `AI_IMPLEMENT_MAX_ITERATIONS`, `AI_IMPLEMENT_MAX_JOB_MINUTES` — but only for a target repo still carrying the legacy `comment-trigger.yml` workflow.

  On a repo synced with the current workflow templates, these mapping fields already apply to `/ai-implement` comment-triggered runs, and the repo variables are read nowhere in that path.
