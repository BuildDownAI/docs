...
<ParamField body="ticketingConfig" type="object">
  ...
  - The three `…Override` fields are optional `customfield_NNNNN` IDs you only need to set when Jira auto-discovery by name is ambiguous — for `AI-Implement Status`, `AI-Implement Repo`, and the multi-select `AI-Implement Profiles` field, respectively. `profilesFieldOverride` also has a dedicated **Profiles Field** dropdown in the admin UI's Jira step, so you rarely need to paste the ID by hand.

  <Note>
    The `AI-Implement Profiles` field is optional. When present, each issue's selected options are forwarded to the implementation run for your own custom pipeline steps to branch on — see [Custom pipeline steps](/latest/customize/custom-steps). If the field is absent, the orchestrator logs a warning and continues without it.
  </Note>
...
<ParamField body="sessionMode" type="string" default="autonomous">
  ...
  <Warning>
    Only takes effect when `executionMode` is `fly-machines`. Under the default `github-actions` execution mode, this setting has no effect and Claude always runs autonomously — setting `interactive` or `hybrid` here will not change that.
  </Warning>
</ParamField>
