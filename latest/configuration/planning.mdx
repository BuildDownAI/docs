...
description: "How the AI planning phase works, what structured planning comments (Implementation Map, Acceptance Bar, and Risks & Open Questions) it posts to your ticketing system, and how to enable, disable, or configure it per project mapping."
---
...
<Steps>
  ...
  <Step title="Claude posts structured planning comments">
    Claude posts exactly three comments on the issue, each with a fixed header that the implementation workflow can locate later:

    - **Implementation Map** (`## 🗺 AI Planning: Implementation Map`) — approach, a Create/Modify/Test/Delete file list, and constraints or hazards the implementer must not break. Cross-story coordination notes fold in here when the issue has dependencies or sibling stories, rather than getting a separate comment.
    - **Acceptance Bar** (`## ✅ AI Planning: Acceptance Bar`) — a numbered list of falsifiable, diff-checkable claims the reviewer can verify.
