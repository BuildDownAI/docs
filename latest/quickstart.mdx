...
<Steps>
  ...
  <Step title="Review and merge the workflow pull request">
    Creating the project mapping opens a pull request in your target repo containing `claude-implement.yml`, `claude-plan.yml`, and a starter `WORKFLOW.md`.

    Merge it before continuing — the workflows have to be on the default branch for AI-Implement to dispatch them.

    <Note>
      The workflow opens PRs using the GitHub App you installed, not the repository's default `github-actions[bot]` actor, so you don't need to enable the target repo's "Allow GitHub Actions to create and approve pull requests" setting. See [Create a GitHub App](/latest/setup/github-app) for the permissions the App needs.
    </Note>
  </Step>
  ...
  <Step title="Mark an issue and watch the PR appear">
    ...
    When the run finishes, the issue is marked ready for review (Linear: `Ready for Review` label; Jira: `AI-Implement Status = PR Ready`), and the PR has an automated review comment saying whether it's ready to merge or still has blocking issues.
...
- Enable the planning phase to have Claude post an implementation map and acceptance bar to your ticketing system before implementation starts.
