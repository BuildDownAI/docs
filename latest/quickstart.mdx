...
<Steps>
  ...
  <Step title="Install dependencies and start the orchestrator">
    ...
    <Tip>
      Want to run the entire loop on your own machine? `npm run dev:local` executes each job locally instead of on GitHub Actions. See [Local development](/latest/setup/deploy#local-development) for what it needs and when to use it.
    </Tip>
  </Step>
