...
<Note>
    Several more skills sit outside that sequence:
    ...
    - `bd-high-plan` settles the shape of a new capability into one planning issue, before `bd-build-up` or `bd-mega-build-up` decomposes any part of it
    - `bd-kg-create`, `bd-kg-refresh`, and `bd-kg-search` build, rebuild, and search a project's optional knowledge graph
    - `bd-system-questions` answers direct questions about the orchestrator itself — why a ticket isn't running, whether a pipeline is stuck, overall health
</Note>
...
graph TD
    ...
    high["bd-high-plan<br/>settle the shape first"]
...
`bd-build-up`, `bd-mega-build-up`, `bd-summit-push`, `bd-build-down`, `bd-super-build-down`, and `bd-smoke-jumper` understand feature-branch grouping. When a parent issue and its children are marked together, they treat the tree as one unit that rolls up to a single reviewable pull request rather than as unrelated issues.
...
| `bd-high-plan` | Settles the shape of a new capability at high altitude — a small set of simple, discrete steps captured as one planning parent issue, before any detailed decomposition. |
...
| `bd-kg-create` | Builds a project's knowledge graph from a template repo — the setup step before `bd-kg-refresh` or `bd-project-setup`'s knowledge-graph binding can use it. |
| `bd-kg-refresh` | Rebuilds a project's knowledge graph so search results reflect the current code and tracker state. |
| `bd-kg-search` | Searches a project's knowledge graph directly, to check what past issues, PRs, decisions, and learnings already exist without running a full build-up or build-down session. |
| `bd-system-questions` | Answers direct questions about the orchestrator — why a ticket isn't running, whether a pipeline is stuck, overall health, the project list, or the current runner mode. |

<Note>
  `bd-kg-create`, `bd-kg-refresh`, `bd-kg-search`, and `bd-system-questions` depend on a project's optional knowledge-graph and orchestrator MCP binding, set up as part of `bd-project-setup`. Each no-ops with a clear message if that binding isn't in place.
</Note>

**Tracker support varies by skill.** `bd-build-down`, `bd-build-up`, and `bd-mega-build-up` carry dedicated Jira handling alongside Linear — `bd-build-up` and `bd-mega-build-up` share one adapter.
