...
**The PR is the work product.** Every run produces a pull request, a [post-push review](/latest/reference/post-push-review), and a ticket-system status change. AI-Implement posts it as a GitHub review with a **Full reviewer report** link and a sticky status comment. Reviewers see exactly what was attempted and where it fell short of the spec.
...
**Gap-fill on demand.** After reviewing a PR, comment `/ai-implement` on it to send Claude back in for a second pass on the same branch.
