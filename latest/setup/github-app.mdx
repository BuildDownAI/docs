...
<Steps>
  ...
  <Step title="Configure webhook delivery">
    Webhook delivery is **effectively required**. AI-Implement's run loop — issue pickup, dispatch, PR opening — is poll-driven and works without it, but two things depend entirely on the webhook:

    - **Automatic follow-up runs on review feedback** — when a reviewer leaves blocking comments on a pull request, AI-Implement picks them up and sends Claude back in without anyone commenting `/ai-implement`.
    - **The `/ai-implement` comment trigger itself.** Workflow sync now removes `comment-trigger.yml` from target repos instead of adding it. The orchestrator's own webhook handles `/ai-implement` comments on synced repos instead, gated by the `GITHUB_WEBHOOK_SECRET` you set below.

    There's no polling fallback for either. Without a working webhook, commenting `/ai-implement` on a synced repo's pull request does nothing.

    Merging a pull request is the exception: it still marks the issue done regardless of webhook configuration, because that path has its own poll-based detector that a webhook only speeds up.

    <Tabs>
      <Tab title="Without webhooks">
        Uncheck **Webhook active**. The event list disappears and there's nothing else to set here.

        Automatic review-feedback follow-up and the `/ai-implement` comment trigger are both unavailable.
      </Tab>
...
## About "Allow GitHub Actions to create and approve pull requests"

You don't need to enable this target-repo setting for AI-Implement. Every synced workflow opens pull requests with a token minted from the GitHub App installation, never the default `github-actions[bot]` identity that this toggle governs, so leaving it off doesn't block PR creation.

<Note>
  Leaving the setting on is harmless. It's just unrelated to AI-Implement — the real prerequisite is the App installed with the repository permissions covered above.
</Note>
