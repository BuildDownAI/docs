...
<Steps>
  ...
  <Step title="Configure webhook delivery">
    Webhook delivery is **required for the `/ai-implement` comment trigger** — commenting `/ai-implement` on a pull request is handled directly by the orchestrator over this webhook, with no polling fallback. It also adds **automatic follow-up runs on review feedback**, again with no polling fallback.

    Everything else is poll-driven and works without a webhook: issues get picked up, runs dispatch, pull requests open, and merging one marks the issue done (merge detection just runs on the next poll instead of immediately).
    ...
    <Tabs>
      <Tab title="Without webhooks">
        ...
        <Warning>
          Without a webhook, commenting `/ai-implement` on a pull request has no effect — there is no other way to trigger a gap-fill run. Only skip the webhook if you don't need that capability.
        </Warning>
      </Tab>
