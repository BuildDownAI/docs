...
<Steps>
  ...
  <Step title="Set repository permissions">
    ...
    AI-Implement reads check runs to wait for an external review, to see failing CI, and to decide auto-merge.

    Without the **Checks** permission, post-push review cannot read check runs, and each run stops with [`CHECKS_PERMISSION_DENIED`](/latest/reference/error-codes).
  ...
  <Step title="Configure webhook delivery">
    Webhook delivery is **optional** for the core run loop — issues still get picked up, runs still dispatch, pull requests still open, and merging one still marks the issue done, all on the poll cycle alone.

    But skipping it disables two things with no fallback: **automatic follow-up runs on review feedback** (when a reviewer leaves blocking comments, AI-Implement normally sends Claude back in without anyone commenting `/ai-implement`), and the **`/ai-implement` comment trigger itself** on repos synced to the current workflow templates — the orchestrator's webhook is the only thing that listens for that comment on such a repo, so without it, commenting `/ai-implement` does nothing.

    <Tabs>
      <Tab title="Without webhooks">
        Uncheck **Webhook active**. The event list disappears and there's nothing else to set here.

        `/ai-implement` and automatic review-feedback follow-ups won't work without a later webhook setup — plan on revisiting this if you need either.
      </Tab>
...
## Add the Checks permission to an installed App

A change to App permissions does not take effect until each installation accepts it.

<Steps>
  <Step title="Set the permission in App settings">
    In the App settings, set **Checks** to **Read-only**, then save.
  </Step>

  <Step title="Accept the new permission on the installation">
    The organization owner opens the installation, then selects **Review request** and **Accept new permissions**.
  </Step>
</Steps>
...
## GitHub Actions pull request setting

The target repo setting **Allow GitHub Actions to create and approve pull requests** is not required.

Every synced workflow opens pull requests with a GitHub App token. The setting governs the separate `github-actions[bot]` identity, which AI-Implement does not use.

Leaving the setting on does no harm.
