...
<Warning>
  Configuring both breaks MCP sign-in for anyone who can only use Microsoft: an [MCP client](/latest/reference/mcp-tools) is never offered a choice, and always signs in with Google. Either provider works on its own.
</Warning>
...
<Tip>
    A connector survives access-token expiry on its own. The server issues refresh tokens, so a connected client refreshes silently in the background rather than sending you back through sign-in when its hourly access token expires.
</Tip>
