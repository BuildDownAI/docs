...
<Steps>
  ...
  <Step title="Review and merge the workflow pull request">
    ...
    If the repo still carries `comment-trigger.yml` from an older sync, this pull request removes it. The orchestrator's own webhook now handles `/ai-implement` PR comments directly for repos on the current templates — see [Create a GitHub App](/latest/setup/github-app) for how to configure webhook delivery.
