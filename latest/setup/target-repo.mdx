...
<Steps>
  ...
  <Step title="Pull request creation setting (not required)">
    The **Allow GitHub Actions to create and approve pull requests** setting is at **Settings → Actions → General** in the target repo.

    This setting is **not required**. Every synced workflow opens pull requests with a GitHub App token, which this setting does not govern.
...
- Read the [post-push review](/latest/reference/post-push-review) on the PR — a GitHub review with a **Full reviewer report** link and a sticky status comment on merge readiness
