...
<Tip>
  AI-Implement marks the originating issue complete for you once the PR merges — no automation to set up on your end.

  **Linear**: the issue moves to the team's Done state and the `Ready for Review` label is removed.

  **Jira**: `AI-Implement Status` is set to `Merged` — add that as an option on the field if you haven't already (see [Jira: manual custom-field setup](#jira-manual-custom-field-setup) above).

  Detection runs on the next poll cycle at the latest; a configured GitHub webhook makes it near-instant. Either way, no action is required from you.
</Tip>
