...
<Steps>
  ...
  <Step title="Review and merge the workflow pull request">
    ...
    <Note>
      If the repo has an old `comment-trigger.yml` file from before the `/ai-implement` comment trigger moved into the orchestrator itself, the sync PR removes it — that's expected, not a mistake in the diff. The trigger keeps working with no workflow file involved; see the [`/ai-implement` comment trigger](/latest/reference/labels#ai-implement-comment-trigger).
    </Note>
