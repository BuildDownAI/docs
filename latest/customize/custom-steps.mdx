...
<AccordionGroup>
  ...
  <Accordion title="dependency-auth">
    Mints a short-lived, read-only cross-repo GitHub token for projects with **Dependency Token Scope** enabled, so the install step can reach private sibling repos. Skipped automatically when the project doesn't have this enabled — see [`dependencyTokenScope`](/latest/configuration/team-repo-mappings#param-dependency-token-scope).

    Override to change how or which credential is minted for dependency access.
  </Accordion>
  ...
  <Accordion title="post-push-review">
    ...
    Two settings you can change without code, both in `.ai-implement/config.yml`: which external reviewers to wait for, via `reviewProviders`, and which CI check-run names count as that external reviewer, via `reviewCheckNames`. The iteration cap and the external-review wait timings are built in; change them by overriding this step.
  </Accordion>
</AccordionGroup>
...
<ParamField body="reviewCheckNames" type="string[]" default="review, code-review-plugin, claude-review, claude code review, claude-code-review">
  Overrides which CI check-run names the `post-push-review` step treats as the external review gate. A check is also matched by default if its name contains both `claude` and `review`, regardless of this list.

  Set this if your repo has an unrelated CI job that happens to match one of the default names — for example a job literally called `review` — so it doesn't get mistaken for the review gate.
</ParamField>
