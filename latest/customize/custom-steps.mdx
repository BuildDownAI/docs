...
<AccordionGroup>
  ...
  <Accordion title="install">
    Installs dependencies (`npm ci`, `yarn install`, or `pnpm install`, chosen by lockfile or an explicit `packageManager` setting) before Claude starts. Also reads `.ai-implement/config.yml` (when present) for repo-level `packageManager`, `models`, and `reviewProviders` settings.

    For a custom shell script instead of a dependency install, use the `setup` step below — that's the one driven by `WORKFLOW.md`'s `setup:` key. Override `install` itself to add cache management, dependency precompilation, or other logic around the install command.
  </Accordion>
  ...
  <Accordion title="preflight">
    Runs your project's `typecheck`, `lint`, and `test` npm scripts after the implement/review loop finishes, so Claude's final diff is checked against your own quality gates before it's pushed.

    Override to change which checks run or how their failures are handled.
  </Accordion>

  <Accordion title="push">
    Commits Claude's changes, pushes the branch to the target repo, and opens the PR — including when the diff was never approved, in which case it opens a **draft PR** carrying the reviewer's final feedback instead of skipping the run entirely. Automatically skipped only on gap-fill runs (an existing PR number is already set), since there's no new branch to push.

    Override to customize commit message format, branch naming, or PR body construction.
  </Accordion>

  <Accordion title="verify">
    Runs the `verify:` shell script declared in your repo's `WORKFLOW.md` front matter, if any. Runs **after** a successful push, with `set -euo pipefail`. Automatically skipped when `verify:` is not declared, or when the diff was never approved (independently of whether push itself ran or was skipped).
...
<Note>
  This mirrors the full built-in step order. If your override omits a step (e.g. `install-skills`, `setup`, `verify`), that step simply doesn't run — trim only the ones you don't need.
</Note>
