...
<AccordionGroup>
  <Accordion title="model">
    ...
    This key is **required when using Bedrock**. There is no Bedrock-specific default — the model resolves from a single provider-agnostic chain that ends in `claude-sonnet-5`.
    ...
    For the Anthropic provider, `claude-sonnet-5` is the intended default and omitting the key is safe.
  </Accordion>
