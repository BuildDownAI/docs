...
<Warning>
  In GitHub Actions execution mode, both `claude-implement.yml` and `comment-trigger.yml` validate the resolved image against an allowlist before running. By default only images from `ghcr.io/builddownai/` pass.

  To use an image from your own registry, add `AI_IMPLEMENT_ALLOWED_RUNNER_IMAGE_PREFIXES` as a repository or organization **variable** (Settings → Secrets and variables → Actions → Variables) with a comma-separated list of allowed prefixes. For example:

  - Single registry: `ghcr.io/your-org/`
  - Multiple registries: `ghcr.io/your-org/,registry.example.com/`

  When the image fails validation, the workflow job fails immediately with an error listing the current allowed prefixes and how to expand the list.

  Fly Machines mode has no such restriction — the orchestrator boots the machine directly on your image without a prefix check.
</Warning>
