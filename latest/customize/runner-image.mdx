...
<Warning>
  The `claude-implement.yml` workflow validates the runner image before starting the job. By default, only images whose name begins with `ghcr.io/builddownai/` are allowed.

  To use an image from your own registry, add a GitHub Actions **variable** (not secret) named `AI_IMPLEMENT_ALLOWED_RUNNER_IMAGE_PREFIXES` to the target repo or your organization, containing a comma-separated list of allowed prefixes.

  Example: `AI_IMPLEMENT_ALLOWED_RUNNER_IMAGE_PREFIXES=ghcr.io/your-org/`

  Without this variable, any image from outside `ghcr.io/builddownai/` causes the validate-runner-image job to fail with a "runner_image is not allowed" error.
</Warning>
