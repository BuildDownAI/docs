...
- **Completion** — a run finishes. Includes the outcome (`completed`, `failed`, `timed_out`, or `review_failed` — the automated review found blocking issues it couldn't resolve within its iteration budget), a link to the pull request if one was opened, a link to the GitHub Actions run, and the duration.
...
<Note>
  On an orchestrator with [self-deploy](/latest/setup/deploy#self-deploy) configured, a fourth notice — **Deployment available** — announces when the tracked branch has moved ahead of the running commit, pointing you at `/admin#deployments` to deploy it.

  This one is optional: without `FLY_DEPLOY_TOKEN` set, it never fires.
</Note>
