...
AI-Implement notifies on three kinds of event.
...
- **Completion** — a run finishes. Includes the outcome (`completed`, `review_failed` — shown as "AI Implementation Needs Review" — `failed`, or `timed_out`), a link to the pull request if one was opened, a link to the GitHub Actions run, and the duration.

**Operational alerts**

- **Stuck run give-up** — a run has failed repeated automated retries and is no longer being retried automatically, so it needs a human to look at it.
- **Reaper burst** — the reaper destroyed an unusually large number of machines in a single sweep, which can signal infrastructure trouble worth checking.
