...
| `POST` | `/trigger/gap-fill` | {/* AUDIT H-2: /trigger/gap-fill description corrected — current comment-trigger.yml does not call this endpoint */} Orchestrator-side gap-fill trigger endpoint, secured by `GAP_FILL_TRIGGER_SECRET`. Returns `503` when the secret is unset. The current synced `comment-trigger.yml` runs gap-fill passes directly via GitHub Actions and does **not** call this endpoint. |
