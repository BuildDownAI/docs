...
Every operator-facing `/api/*` endpoint except `/api/auth` requires a `Bearer` token obtained from `/api/auth`. Tokens expire after 24 hours. The session-machine routes at the end of this section are the exception — they authenticate by a per-session nonce instead.
...
<Note>
  `/api/auth` is the only *operator-facing* endpoint that does not require a bearer token — every other route documented on this page returns `401 Unauthorized` if the token is missing or expired.
</Note>

<Warning>
  Bearer tokens are not a blanket gate on `/api/*`: a separate handful of routes used by runner sessions authenticate by a different mechanism, and the GitHub webhook endpoint authenticates by signature. Account for all three when writing ingress rules. See [Session-machine endpoints](#session-machine-endpoints) below.
</Warning>
