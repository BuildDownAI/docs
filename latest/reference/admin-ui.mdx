...
<Info>
  **New in latest:**
  ...
  - Two more optional fields are available on the mapping: **Branch Prefix** (a path segment prepended to generated branch names) and **Skills Repo** (a repo to clone for Claude skills at the start of each run). See [Project mappings](/latest/configuration/team-repo-mappings#param-branch-prefix) for both.
</Info>
...
<Note>
  The sidebar's **Secrets** entry itself is a placeholder — it points you to the two panels below rather than hosting its own browsing UI. Manage secrets from **Projects** (per-team) and **Settings** (global).
</Note>

#### Per-team secrets

Each project mapping has its own per-team secrets, managed from a **Secrets** action on that project's row in the **Projects** panel.
...
Global secrets are Fly app secrets that do not belong to any specific project mapping. They are listed, created, and deleted from the **Settings** panel, alongside the Fly sessions app configuration.
