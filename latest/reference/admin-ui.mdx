...
Unparking clears the park and resets the failure counter, so the orchestrator dispatches the issue again on the next poll. See [An eligible issue stopped being dispatched](/latest/reference/troubleshooting#an-eligible-issue-stopped-being-dispatched) for what parks an issue and what unparking does and does not change.
