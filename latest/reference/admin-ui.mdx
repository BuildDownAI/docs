...
<Info>
  **New in latest:**
  - Three optional **run-cap** fields — `maxTurns`, `maxIterations`, and `maxJobMinutes` — are now available per project, in the **Edit** dialog on an existing project's row. Tune these to control cost and reliability. They aren't part of the creation stepper; set them after the project exists.
...
<Tip>
  The Capacity step in this stepper only sets **Max parallel AI issues**. The per-run caps — `maxTurns`, `maxIterations`, and `maxJobMinutes` — aren't part of project creation; set them afterwards from the project row's **Edit** dialog. Defaults if left blank: 50 turns, 3 iterations (2 for Bedrock), 90 minutes.
</Tip>
...
All `/api/*` endpoints except `/api/auth` and `/api/auth/*` require a `Bearer` token. Signing in through the browser UI (SSO or access code) sets this up for you automatically; the flow below is for scripting against the API directly, which only the access-code path supports. Tokens expire after 24 hours.
...
<Steps>
  <Step title="Obtain a bearer token">
    ...
    <Note>
      This is the access-code path. If your orchestrator only has SSO configured (no `ADMIN_ACCESS_CODE`), obtain a session by signing in through the browser UI instead — there is no scriptable token exchange for SSO.
    </Note>
  </Step>
...
| `GET` | `/api/auth/providers` | List which SSO providers are configured |
| `GET` | `/api/auth/:provider/start` | Begin the SSO sign-in redirect for `google` or `microsoft` |
| `GET` | `/api/auth/:provider/callback` | SSO provider redirects here after sign-in; sets the session cookie |
| `POST` | `/api/auth/logout` | End the current SSO session |

<Note>
  `/api/auth` and `/api/auth/*` are the only endpoints that don't require a bearer token — the SSO routes issue a session cookie instead. All other `/api/*` routes return `401 Unauthorized` if the token is missing or expired.
</Note>
