...
<Note>
  Four of these are placeholders for planned functionality and have no section below:
  - **Triggers & channels**
  - **Policies & risk**
  - **Webhooks**
  - **Updates**.
...
Three panels are live but not yet documented in full here:
...
<Steps>
  ...
  <Step title="Fill in Ticketing System and Source">
    ...
    Once the owner and repository are filled in, **Check installation** confirms the GitHub App can reach that repo. It reports one of three outcomes:
...
Operator-facing `/api/*` endpoints are authenticated by a session. Requests carry the session in an httpOnly cookie set at sign-in, and an `Authorization: Bearer` header holding the same token is accepted as a fallback for compatibility.

A session is created when you sign in, either through single sign-on or by submitting the access code.

Sessions last 24 hours.

The sign-in routes under `/api/auth/*` — the provider list, sign-out, and each provider's start and callback — are reachable without a session, so signing in works even when no access code is configured.

The session-machine routes at the end of this section authenticate differently, by a per-session nonce.

### Authentication

The browser signs you in through single sign-on or the access code and keeps the session in a cookie for you. For scripting against the API, obtain a token with the access code and pass it as a bearer header:
...
<Note>
  The `/api/auth/*` sign-in routes above are reachable without a session — they are how you get one, and they stay open even when no access code is configured. Every other operator-facing route on this page returns `401 Unauthorized` when the session cookie or bearer token is missing or expired.
</Note>

<Warning>
  A session is not a blanket gate on `/api/*`: a separate handful of routes used by runner sessions authenticate by a different mechanism, and the GitHub webhook endpoint authenticates by signature. Account for all three when writing ingress rules. See [Session-machine endpoints](#session-machine-endpoints) below.
</Warning>
...
Three more `/api/*` routes are called by Fly Machine and local Docker runner sessions, not by operators, and — unlike the rest of `/api/*` — do **not** require a bearer token. Each authenticates with a per-session nonce instead, issued when the session starts and rejected once it expires.
...
Five endpoints are called by **runners** (GitHub Actions workflow jobs, Fly Machine sessions, or local Docker containers running an issue), not by operators. Three sit outside the `/api/*` namespace and two sit inside it, but all are authenticated by a runner token rather than by an operator session.
...
All five endpoints respond `501 Not Implemented` when the orchestrator has no `RUNNER_TOKEN_SECRET` configured, since none can validate tokens without it.
