...
Fleet-wide outcome metrics over a selectable window (7, 30, or 90 days).

- **Fleet by repo** — per-repo job and issue counts, completed/failed/merged totals, average review passes, and cost in USD.
- **Outcomes** — one-shot approval rate (approved on the first dispatch), eventual approval rate (approved across all dispatches on an issue), and escape rate (human edits made after merge).
- **Planning A/B** — the same job/pass/cost/merge metrics split by whether the issue went through the planning phase, so you can see what planning buys you.
- **Runaways** — issues with three or more dispatches, or three or more consecutive failures.
...
<Steps>
  ...
  <Step title="Step through the remaining configuration">
    ...
    The **Capacity** step sets `maxInProgressAiIssues` — how many issues this mapping can run at once. The per-run caps (`maxTurns`, `maxIterations`, `maxJobMinutes`) aren't in the stepper; set them afterwards from the project's **Edit** dialog. See [Project mappings](/latest/configuration/team-repo-mappings#capacity) for the full field reference and defaults.
...
High-level runner-infrastructure dashboard, and home of the **runner mode toggle** — the global failover control, offering four options: `default`, `gha`, `fly`, and `shadow`.

`local` is a valid runner mode too, but the toggle doesn't expose it — set it through the environment or the API instead.

<Note>
  If the `RUNNER_MODE` environment variable is set on the orchestrator process, it takes precedence over this database value and the UI flags that an env-var lock is active. Changing the mode via the UI or API while the env var is set persists the new value to the database, but it has no effect until the env var is removed and the orchestrator is restarted.
</Note>
...
A **parked issues** card lists issues the dispatch circuit breaker has given up on after repeated consecutive failed dispatches for the same issue and phase, with an **Unpark** action to let the next poll try again. See [`DISPATCH_BREAKER_THRESHOLD`](/latest/configuration/environment-variables) to change how many consecutive failures trip it.
...
Self-deploy status and controls for this orchestrator, when it's running on a Fly deployment stamped with the `AI_IMPLEMENT_SOURCE_*` build args (see [`AI_IMPLEMENT_SOURCE_COMMIT`](/latest/configuration/environment-variables)).

KPI cards show:
- **Availability** — whether a newer commit is available on the watched source branch
- **Last deploy** — outcome, commit, and how long ago it ran

You can trigger a deploy on demand. New dispatches pause immediately and in-flight work drains before the build starts. A policy card lets you turn on automatic deploy when a new commit becomes available, and/or a notification to the configured webhook when one does.

If the orchestrator bundles a knowledge-graph sidecar, a **Knowledge graph** card shows the served graph's snapshot age and lets you trigger a refresh — this re-fetches the KG source repo's committed snapshot and restarts the sidecar without a full deploy or a dispatch pause.

<Note>
  If `FLY_DEPLOY_TOKEN` is unset, or the running image wasn't built with the source-stamp build args, this panel shows self-deploy as not configured rather than a broken deploy control.
</Note>
...
<Note>
  The runner mode toggle lives on the **Runners** panel — see [Runners](#runners) above.
</Note>
...
The sign-in allowlist and role/page-grant editor — this panel, not `OAUTH_ALLOWED_DOMAINS` / `OAUTH_ALLOWED_EMAILS`, is the durable source of truth once anyone has signed in. See [Set up admin sign-in](/latest/setup/sso) for how the two relate.

The **sign-in allowlist** table lists every entry (domain or individual email), its role, and when it was added. Adding an entry from here:
- A **domain** admits every address at that domain, signed in as `user`.
- An **email** can be `admin`, and an email entry overrides the domain's role for that one person.

A **manage user page access** editor sets which admin pages the `user` role can open, beyond the MCP server it always reaches. The same set of granted pages applies to every `user`; some pages (anything that reads runner environment values or controls infrastructure) can never be granted, and the editor states why for each.

A **recent access changes** table records who changed the allowlist, when, and what changed.

<Warning>
  If the allowlist can't be read — a database fault, for example — this panel shows everything as read-only and sign-in is denied for everyone until it recovers. Nothing you had unsaved is lost.
</Warning>

<Note>
  Signed in with the shared access code rather than SSO, this panel is read-only: an access-code session can't be attributed to a person, so it can't be the one to make an attributed change. Sign in with SSO to edit the allowlist.
</Note>
...
An issue enters the dedup window on dispatch and stays there to prevent the orchestrator from picking it up again. There is no time-based expiry — an entry clears only when a run fails or times out, when the reconcile loop reaches a terminal state for the issue, or when you remove it manually here.

You can manually remove an issue from the dedup window if you want the orchestrator to re-dispatch it.
...
Most `/api/*` endpoints require a `Bearer` token obtained from `/api/auth`. Tokens expire after 24 hours.

A handful of endpoints intentionally bypass this: the callback and status routes runner sessions call directly (`/api/token`, `/api/runner/dependency-token`, `/api/runner/publication-token`, `/api/status`, `/api/step-report`) authenticate with their own per-run tokens instead, and `/api/github/webhook` authenticates with an HMAC signature. See [Runner callback endpoints](#runner-callback-endpoints) and [Webhook and trigger endpoints](#webhook-and-trigger-endpoints) below.
...
<Note>
  `/api/auth` is the only *admin panel* endpoint that does not require a bearer token. Runner-callback and webhook routes use their own separate authentication instead — see the note above. Every other `/api/*` route returns `401 Unauthorized` if the bearer token is missing or expired.
</Note>
...
#### Deployments, access, and reports

| Method | Path | Description |
|---|---|---|
| `POST` | `/api/deploy` | Trigger a self-deploy: pause dispatch, drain in-flight work, then build and release |
| `GET` | `/api/deployment-status` | Get availability (is a newer commit ready to deploy) and the last deploy outcome |
| `POST` | `/api/deploy-policy` | Set auto-deploy and deploy-notification policy |
| `GET` | `/api/access` | Get the sign-in allowlist and recent access-change history |
| `POST` | `/api/access` | Update the sign-in allowlist |
| `GET` | `/api/access-grants` | Get which admin pages the `user` role can open |
| `POST` | `/api/access-grants` | Update which admin pages the `user` role can open |
| `GET` | `/api/report` | Fleet outcome metrics for a window (`?days=7\|30\|90`) — per-repo counts, one-shot/eventual/escape rates, planning A/B, runaways |
