...
<AccordionGroup>
    ...
    <Accordion title="Blocked by a security guardrail" description="A staged change looked like a secret, so the push was blocked">
        ...
        A file is flagged when it falls into one of these categories:

        - **Environment and config files** — for example `.env`, `secrets.yml`, or a `credentials.yml.enc`.
        - **Private keys and certificates** — for example `.pem` and `.key` files, or an SSH key like `id_rsa`.
        - **Cloud credentials** — for example an AWS `credentials` file, a GCP service-account JSON, or a `.tfstate`.
        - **Database credentials** — for example a `.pgpass` file or a `database.yml`.
        - **Tokens and auth files** — for example `.npmrc`, `.netrc`, or a `kubeconfig`.
