...
A GitHub Actions dispatch is treated as **overdue** once it has stayed non-terminal for its configured **Job Timeout (min)** plus a short grace period. [The run hit the time limit](/latest/reference/troubleshooting#the-run-hit-the-time-limit) covers where that timeout is set and how to raise it.
...
A spent budget does not clear on its own. The issue stays out of the automatic queue until an operator re-applies the trigger, as [Triggers and status indicators](/latest/reference/labels) describes.
