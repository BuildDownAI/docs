...
Most reasons clear on their own once the condition changes — a slot frees up, or the dispatch entry clears — but `no-mapping` needs you to add a project mapping.
...
| `dedup` | Already dispatched recently. Waiting for the in-flight job. | The issue already has a dispatch entry that hasn't cleared. There is no time-based expiry: it clears when the run fails or times out, when the reconcile loop reaches a terminal state for the issue, or through a manual delete in the admin UI. |
