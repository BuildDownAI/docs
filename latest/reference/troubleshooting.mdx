...
<AccordionGroup>
  ...
  <Accordion title="/ai-implement comment does nothing">
    ...
    1. **The orchestrator's webhook isn't configured.** The `/ai-implement` trigger is handled by the orchestrator over a GitHub webhook, not by a workflow file in the target repo. Check that `GITHUB_WEBHOOK_SECRET` is set on the orchestrator and that the GitHub App's webhook is active and subscribed to `issue_comment` events — see [Create a GitHub App](/latest/setup/github-app).
