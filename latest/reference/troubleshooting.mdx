...
<AccordionGroup>
  ...
  <Accordion title="A Jira issue ran despite an open blocker">
    The orchestrator skips an issue while any issue linked as blocking it is still open, so a run that starts while a blocker is unresolved means the block was never seen.

    On Jira the check recognizes only the default `Blocks` link type, matched by that name. An instance that renamed the link type gets no blocker check, and the issue dispatches as though nothing blocks it.

    **Nothing marks the miss.** There is no error and no log line, so a run that should have waited is the only signal you get.

    Linear is unaffected. Its blocking relation is structural rather than matched by a name, so a renamed link type cannot silence it.

    To restore the check, either:

    - Rename the link type back to Jira's default `Blocks`, or
    - Stop relying on the blocker skip for that project and gate the issue through your mapping's JQL instead.
  </Accordion>
