...
<AccordionGroup>
  ...
  <Accordion title="/ai-implement comment does nothing">
    ...
    1. **The webhook isn't configured.** On a repo synced with the current workflow templates, `/ai-implement` comments are delivered to the orchestrator through its GitHub webhook, not a workflow file in the target repo. Confirm `GITHUB_WEBHOOK_SECRET` is set on the orchestrator and that the App's webhook is active and subscribed to **Issue comment** events — see [Create a GitHub App](/latest/setup/github-app). A repo that hasn't synced since this changed still relies on the legacy `comment-trigger.yml` workflow instead. Running **Sync workflows** on that repo now removes the file rather than installing it, so don't use that action expecting it to add the trigger.
