  ...
  <Accordion title="Fly Machine session stuck or not terminating">
    ...
    <Steps>
      ...
      <Step title="Destroy the stuck session">
        ...
        Destroying the session also clears the in-progress marker on the associated issue (`AI-Working` on Linear, or `AI-Implement Status` set to `Implementing` on Jira) and removes the issue from the 24-hour dedup window.
      </Step>
