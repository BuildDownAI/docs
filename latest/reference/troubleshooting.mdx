...
<AccordionGroup>
  ...
  <Accordion title="/ai-implement comment does nothing">
    ...
    1. **`comment-trigger.yml` is not installed.** The workflow must exist at `.github/workflows/comment-trigger.yml` in the target repo. It arrives with the workflow sync — use the **Sync workflows** action on the project's row in the admin UI, then merge the pull request it opens.
  ...
  <Accordion title="The same issue is being picked up repeatedly">
    ...
    - The issue reaching a terminal state (completed or cancelled), which the orchestrator notices on its next poll — this is what clears the entry once its pull request merges and the issue closes
    - The issue no longer being found in the tracker — deleted, or no longer returned by any provider
    - A failed or timed-out run clearing its own entry, but only while the issue is still within its retry budget. Once the budget is exhausted, the entry is kept and the issue is parked for a human instead of retried.
    - A session being destroyed from the sessions panel
    - The reaper tearing down a session because it ran past the maximum session age, or because its issue had already reached a terminal state
    - An operator clearing it by hand
    ...
    - If the issue is not in the dedup window, one of the causes above cleared its entry and the orchestrator is treating the issue as new.
