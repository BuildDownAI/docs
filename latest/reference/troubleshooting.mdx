...
<AccordionGroup>
  ...
  <Accordion title="/ai-implement comment does nothing">
    ...
    5. **The comment does not start with `/ai-implement`.** The trigger requires the comment body to start with `/ai-implement` — a comment that only mentions the token partway through, or one that runs `/ai-implement` straight into other characters with no space (e.g. `/ai-implementplease`), is ignored.

    <Tip>
      Text after `/ai-implement` is not just tolerated — it's forwarded to Claude as an instruction for that pass (e.g. `/ai-implement focus only on the validation logic`), taking precedence over the default gap-fill behavior where the two conflict.
    </Tip>
  </Accordion>
  ...
  <Accordion title="Reached max turns">
    ...
    - **How to fix it.** Raise the budget with the **Max Turns** field on the project mapping (Configure → Projects in the admin UI). For `/ai-implement` comment-triggered runs, set `AI_IMPLEMENT_MAX_TURNS` on the target repository instead. A higher budget lets one run do more work before it stops.
