...
<AccordionGroup>
  ...
  <Accordion title="/ai-implement comment does nothing">
    ...
    1. **The orchestrator's webhook is not configured.** The trigger is served by the orchestrator, not by a workflow in your repo, so it needs `GITHUB_WEBHOOK_SECRET` set on the orchestrator and the GitHub App's webhook active and subscribed to issue comments. Nothing polls as a fallback, so an unconfigured webhook means the comment is never seen.
    2. **The PR was not opened by AI-Implement.** A gap-fill run continues an existing run, so there has to be one on record. On a pull request the orchestrator has no record of, it replies saying so and asks you to start a run from the issue tracker instead.
    3. **The repo has no project mapping, or its project is paused.** The comment is matched against the configured projects; one that matches nothing, or matches a paused project, is dropped without a reply.
