...
<Tabs>
  ...
  <Tab title="Jira">
    ...
    <Note>
      Jira gates on blocking links the same way Linear does: an issue linked with a `Blocks` relation to another issue that isn't in a Done-category status is skipped until that link clears.

      The match is on Jira's default `Blocks` link type name — an instance that has renamed it will fail open, dispatching the issue as though it weren't blocked.

      You can also keep an issue out of the poll by narrowing your JQL, or by moving it to any status other than `Ready` or `Plan Approved` — those are the only two values the poll accepts.
    </Note>
...
<Tip>
  A repo synced with the current workflow templates doesn't carry `comment-trigger.yml` — the orchestrator's own webhook picks up the `/ai-implement` comment directly instead.

  This requires [`GITHUB_WEBHOOK_SECRET`](/latest/configuration/environment-variables#param-github-webhook-secret) to be configured and the GitHub App's webhook events set up; see [Create a GitHub App](/latest/setup/github-app).
</Tip>

<Note>
  A repo that hasn't synced since this changed still carries `comment-trigger.yml`, which handles the trigger on its own instead. Use the **Sync workflows** action on its project row in the admin UI to remove the old file and move the repo onto the webhook path.
</Note>
