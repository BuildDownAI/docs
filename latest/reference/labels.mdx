...
<Tabs>
  <Tab title="Linear">
    ...
    | `Ready for Review` | Orchestrator (auto) | When a PR is opened after implementation | Not removed automatically |
    | *(Done state)* | Orchestrator (auto) | When the PR merges | — (terminal) |
    ...
    When the PR merges, the orchestrator moves the issue to your team's Done state and removes the `Ready for Review` label — no action needed from you. This is detected on the next poll cycle at the latest; a configured GitHub webhook makes it near-instant. It runs even for paused project mappings.
  </Tab>
  ...
  <Tab title="Jira">
    ...
    When the PR merges, the orchestrator sets `AI-Implement Status` to `Merged` — no action needed from you, beyond adding `Merged` as an option on the field during setup (see [Jira: manual custom-field setup](/latest/setup/target-repo#jira-manual-custom-field-setup)). This is detected on the next poll cycle at the latest; a configured GitHub webhook makes it near-instant. It runs even for paused project mappings.
  </Tab>
</Tabs>
...
<Tip>
  Add text after `/ai-implement` to give Claude a specific instruction for that pass — for example, `/ai-implement focus only on the validation logic, skip the tests`. The instruction takes precedence over the default gap-fill behavior wherever the two conflict. A bare `/ai-implement` with nothing after it runs the standard gap-fill pass.
</Tip>
