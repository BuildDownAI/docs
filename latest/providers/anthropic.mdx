...
1. **Bedrock** — used if the orchestrator sets `provider=bedrock` (see [AWS Bedrock](/latest/providers/aws-bedrock))
...
- The gap analysis step runs on that same model. There is no separate, cheaper default for it, and `gap_analysis_model:` is not a recognized front-matter key.
