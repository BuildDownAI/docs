...
description: "The end-to-end flow from marking an issue in Linear or Jira to a merged PR, including planning, labels, the post-push review, and gap-fill runs."
---
...
<Steps>
  ...
  <Step title="Orchestrator polls for marked issues">
    ...
    A concurrency limit applies: by default, no more than 3 issues can be in progress at once. You set the limit per mapping in the admin UI, and eligible issues wait until a slot opens.
  ...
  <Step title="Issue is picked up and marked In Progress">
    <Tabs>
      <Tab title="Linear">
        When the orchestrator selects an issue, it adds the **AI-Working** label, which signals to your team that a run is active on this issue. It also moves the issue to **In Progress**, unless you have already started it yourself.
      </Tab>
    ...
    - **Already dispatched** — even after a job finishes, the same issue won't be re-dispatched while a dedup entry from its previous dispatch is still on record.

    A separate condition also stops a dispatch: an issue the dispatch breaker has [parked](/reference/admin-ui#parked-issues) after repeated failed dispatches is skipped until an operator unparks it.

    Any of these is enough to skip dispatch.
  </Step>

  <Step title="Planning phase (if enabled)">
    If planning is enabled for the project mapping (the default), the orchestrator dispatches a separate planning workflow (`claude-plan.yml`) before implementation begins, and labels the issue **AI-Planning** while it runs.

    Claude reads the codebase in read-only mode, guided by your repo's `PLANNING.md` template, then posts structured comments to the issue:

    - **Implementation Map** — the approach, the files it expects to create, modify, test or delete, and the constraints an implementer must not break.
    - **Acceptance Bar** — numbered claims that can be checked against the diff, or by running a command.
    - **Risks & Open Questions** — edge cases and unknowns found while exploring the codebase.

    Each lands on the issue for you to read. The orchestrator also reads the Acceptance Bar, which the review pass checks the diff against, and the Implementation Map's file list, which is what holds back a sibling issue whose work would overlap.
  ...
  <Step title="The pull request is reviewed">
    Once the pull request is open, AI-Implement reviews it from the outside — reading the diff as pushed, commenting what it finds, and pushing fixes where it can.

    Every comment it leaves ends with a merge-readiness line:
    - `Ready to merge.` means a review pass found nothing left to fix.
    - Anything else means the pull request still needs someone to look at it, and [Post-push review](/reference/post-push-review) covers each of those cases.
  </Step>
  ...
  <Step title="Issue is updated to Ready for Review">
    ...
    Your team can now review the PR in GitHub, along with the review comments on it, and see the status change in your ticketing system — no context-switching required.
  </Step>

  <Step title="Gap-fill runs with /ai-implement">
    <Warning>
      This step and the next both need a GitHub webhook pointed at your orchestrator, which serves them and polls for neither. On a repo synced to the current workflow templates, nothing else watches for an `/ai-implement` comment, and review feedback never starts a follow-up run without it.

      See [`GITHUB_WEBHOOK_SECRET`](/configuration/environment-variables#param-github-webhook-secret) for the setup.
    </Warning>

    After reviewing the PR, you can send Claude back in for another pass without starting a new run. Comment `/ai-implement` on the PR (in any PR comment), and the orchestrator picks it up.

    Anything you write after the trigger becomes the brief for that run, as in `/ai-implement also cover the error path with tests`. Leave it bare and Claude works from the original issue alone.

    Claude checks out the existing branch, reads the same `WORKFLOW.md` template, and pushes new commits to the same branch.
  ...
  <Step title="Late review feedback handling">
    ...
    Each new piece of feedback is deduplicated against what Claude already addressed in earlier passes, then queued for the next follow-up run. The run pushes to the same PR branch — no new PRs or branches.

    <Tip>
      To make AI-Implement wait for an external review bot's findings before running its own review (so both reviewers' feedback get folded into one pass), configure the `reviewProviders` field in `.ai-implement/config.yml`.
      
      See [Repo-level configuration file](/customize/custom-steps#repo-level-configuration-file).
    </Tip>
  </Step>
  ...
  <Step title="PR merges and the issue is marked done">
    ...
    <Tabs>
      ...
      <Tab title="Jira">
        ...
        <Warning>
          `Merged` must exist as an option on that field before your first run — see [Jira custom-field setup](/setup/target-repo#jira-manual-custom-field-setup).
        </Warning>
      </Tab>
    </Tabs>
...
<Note>
  This is the lifecycle for a single issue. When a parent issue and its children are marked together, the children's pull requests target a shared branch instead of your default branch, and the whole tree lands as one pull request at the end — see [Feature-branch grouping](/reference/feature-branch-grouping).
</Note>
