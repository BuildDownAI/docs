...
<Steps>
  ...
  <Step title="Gap analysis is posted to the PR">
    ...
    Gap analysis runs on the same model as the rest of the run — the `model:` value from `WORKFLOW.md`, or `claude-sonnet-4-6` when that's unset. There is no separate, cheaper default for this step.
  </Step>
  ...
  <Step title="Gap-fill runs with /ai-implement">
    After reviewing the PR and reading the gap analysis, you can send Claude back in for another pass without starting a new run. Comment `/ai-implement` on the PR (in any PR comment), and the orchestrator's GitHub webhook picks it up and dispatches a gap-fill run.
    ...
    <Warning>
      This needs the same GitHub webhook that late review feedback handling (below) uses. Without one configured, `/ai-implement` comments are never picked up — see [`GITHUB_WEBHOOK_SECRET`](/configuration/environment-variables#param-github-webhook-secret).
    </Warning>
  </Step>
