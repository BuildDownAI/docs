...
<Note>
  `bd-project-setup` binds either tracker. `bd-smoke-jumper` needs a tracker connection too — to read issue context and file the failures it finds — but treats Linear and Jira identically rather than branching behavior by tracker. `bd-belay-on` is the only skill with no tracker dependency at all.
</Note>
