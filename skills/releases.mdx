...
<Note>
  **These numbers track delivery, not releases.** A version bump is how a skill change reaches an installation at all, so the plugin's version moves on every change.

  Most of those numbers are never published. The list below skips, and a missing number means no stable-channel install ever ran it. It also means the sequence climbs faster than [AI-Implement's](/releases), so the two cannot be compared.
</Note>
