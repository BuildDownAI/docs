...
The variables below are substituted into the body by a regular expression that matches a fixed list of `${NAME}` tokens — not by `envsubst`, so shell semantics don't apply. Reference any of them anywhere in the body.

Any other `${UPPER_SNAKE}` token is replaced with an empty string, even one you never meant as a variable. A shell example containing `${...}` — a parameter expansion, say — is silently emptied, with no error and no log line.
...
**Do NOT create a new branch or PR. Do NOT commit or push.** Leave your file
changes unstaged and uncommitted — the AI-Implement pipeline commits and
pushes them to the existing PR branch after review passes.
