...
- **Skills installation and dependency tokens**, both of which a project mapping configures. There is no mapping.
...
<Steps>
  <Step title="Start Docker">
    The run executes in a container. [Local development](/setup/deploy#local-development) covers starting Docker for the orchestrator, and the same applies here.
...
- **What the run thought it did** — `ai-output/comments/01-summary.md` in the workspace, written the way a pull-request body would be, alongside the run's statistics.
...
<ParamField body="profiles" type="array">
  [Profile names](/customize/custom-steps#profiles) carried into the run for custom steps to branch on. Set this only if your own steps expect it.

  They work here whichever ticketing system you use, because nothing reads a ticket. A dispatched run takes them from a Jira custom field instead, so a Linear project can exercise a profile here but never set one on a real issue.
</ParamField>
...
The run writes into the workspace as well: `ai-output/comments/` holds the summary the run would have posted to a ticket, alongside the run's statistics. That summary is the readable account of what happened, and usually the first thing worth opening — but you will not find it through `git status`, because the run adds `ai-output/` to the workspace's `.git/info/exclude` so its own scratch files can never be staged.
...
Both flags apply to implementation runs only.
...
| Option | Effect |
|---|---|
| `--workspace`, `-w` | The repository the task operates on. Required. |
| `--task`, `-t` | The task file to run. Required. |
| `--phase` | `implementation` (the default), `planning`, or `full`. |
| `--image` | The runner image to use. Defaults to `LOCAL_RUNNER_IMAGE`, then to `ai-implement-runner:local`. |
| `--until` | Stop after this step. Implementation runs only. |
| `--shell` | Open a shell in the container when the run finishes. Implementation runs only. |
