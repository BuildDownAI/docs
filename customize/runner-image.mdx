...
- **Published by the target repository itself** — no configuration. A package is linked to the repository that publishes it, that repository's workflows inherit access, and the image allowlist trusts the owner's own namespace automatically, so no allowlist entry is needed either.
...
<Steps>
  ...
  <Step title="Build and push the image">
    Build the image and push it to a registry AI-Implement can reach. GitHub Container Registry (`ghcr.io`) is a common choice.
    ...
    A public image works in either execution mode.

    For a private image, the requirements differ by mode — see [How it works](#how-it-works).
  </Step>
