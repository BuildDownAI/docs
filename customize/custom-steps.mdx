...
<AccordionGroup>
  ...
  <Accordion title="dependency-auth">
    Fetches a short-lived, `contents: read` token scoped to the GitHub App installation and installs it as a git credential helper, so the `install` step can resolve private sibling-repo dependencies. Only runs when the project's [Dependency Token Scope](/configuration/team-repo-mappings#param-dependency-token-scope) is set to `installation`; otherwise skipped.

    Override to change how dependency credentials are vended — for example to add a package-registry-specific credential alongside the git one.
  </Accordion>
...
<ParamField body="packageManager" type="string" default="auto-detected">
  ...
  A repo with no `package.json` skips installation entirely, so a non-Node project needs no setting here. Any value outside the three above fails the run.
</ParamField>
...
<ParamField body="reviewProviders" type="string[]">
  ...
  - **Omitted** — the step waits for an external review check and merges its findings in, matching against the default check names below.
...
<ParamField body="reviewCheckNames" type="string[]" default="review, code-review-plugin, claude-review, claude code review, claude-code-review">
  Overrides which check-run names the `post-push-review` step treats as the external review gate, when `reviewProviders` isn't an empty list.

  Beyond the listed defaults, a check whose name contains both "claude" and "review" also matches. Set this list if your repo runs an unrelated check named `review` or `code-review-plugin` — otherwise that check becomes the review gate by accident.

  Setting this replaces the defaults entirely, including the "claude" + "review" fallback — matching becomes exact against only the names you list.
</ParamField>
