...
{/* AUDIT H-1: Secrets panel is a stub; per-team secrets are on Projects page, global secrets on Settings page */}
### Secrets

<Note>
  The standalone Secrets panel is not yet available. Until it ships, secrets are managed from two other panels:

  - **Per-team secrets** — open the **Projects** panel, find the project row, and click **Secrets**. This inline panel lets you view, add, and delete secrets scoped to that project.
  - **Global machine secrets** — open the **Settings** panel and scroll to the **Global Machine Secrets** section.
</Note>

Secrets are stored as Fly app secrets on the sessions app. Per-team secrets use the project key as a prefix — for example, a secret named `MY_TOKEN` for project `ENG` is stored as `ENG_MY_TOKEN`. Global machine secrets have no prefix and are injected into every session machine.

<Warning>
  Secret names must contain only letters, digits, and underscores. Global secret names must not start with any project-key prefix — those names are reserved for per-team secrets.
</Warning>

{/* AUDIT H-1: Settings panel also manages global machine secrets — not documented */}
### Settings

The settings panel manages two areas:

**Fly Sessions App configuration:**
...
The lower half of the Settings panel is the current home for global machine secrets — Fly app secrets injected into every session machine. Add secrets here instead of using the Fly CLI, and delete them here to revoke them. Values are write-only; they cannot be read back through the UI.

<Warning>
  Global secret names must not start with any project-key prefix (e.g. `ENG_`). Names matching a team prefix are reserved for per-team secrets, managed from the Projects panel.
</Warning>
