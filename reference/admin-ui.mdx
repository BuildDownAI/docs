...
<Note>
  The dedicated Secrets sidebar item is not yet active — it shows a placeholder page. Secret management is split between the [Settings panel](#settings) and the [Projects panel](#projects-team%2Frepo-mappings).
</Note>
...
The Settings panel is also where you manage global Fly app secrets — values injected into every session machine as environment variables. Use this for credentials shared across all projects. Per-project secrets are managed from each project row in the [Projects panel](#projects-team%2Frepo-mappings).
