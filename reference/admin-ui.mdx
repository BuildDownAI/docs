...
The top of the page hosts the **runner mode toggle** (`default` / `gha` / `fly` / `local` / `shadow`).
...
The two `POST` endpoints respond `501 Not Implemented` when the orchestrator has no `RUNNER_TOKEN_SECRET` configured, since neither can validate tokens without it.
