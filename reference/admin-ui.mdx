...
An issue enters the dedup window when it's dispatched, which stops the orchestrator from picking up the same issue repeatedly.

The entry stays until something clears it, rather than expiring on a timer.

You can manually remove an issue from the dedup window, so the orchestrator can pick it up again on the next poll.
