...
Fleet-wide delivery metrics across every project mapping, for a selectable trailing window (7, 30, or 90 days).

A per-repo table shows job and issue counts, completions, failures, merges, average review passes, and cost in USD.

Three outcome rates summarize the whole fleet: **one-shot** (approved on the first dispatch), **eventual** (approved across all dispatches, however many it took), and **escape rate** (the share of merged PRs a human had to edit afterward).

A planning A/B table compares planned issues against unplanned ones on the same metrics, so you can see whether the planning phase is paying for itself.

A runaways table lists issues with three or more dispatches, or three or more consecutive failures — the ones worth a manual look before they keep burning turns.
...
After enough consecutive dispatch failures for the same issue and phase, the orchestrator stops retrying it automatically and parks it — this section lists every parked issue, when it was parked, and its last failure conclusion.

<Note>
  A parked issue also gets a tracker comment naming the failure count and linking the failed runs, so the notice reaches the ticket even if nobody is watching this panel.
</Note>

Unpark an issue from its row to let the orchestrator dispatch it again on the next poll. See [`DISPATCH_BREAKER_THRESHOLD`](/configuration/environment-variables#param-dispatch-breaker-threshold) for the failure count that triggers parking.
...
Lets this orchestrator redeploy itself on Fly.io when a newer image becomes available, without a manual `fly deploy`.

<Note>
  Self-deploy needs `FLY_DEPLOY_TOKEN` set and the running image built with its `AI_IMPLEMENT_SOURCE_*` build args stamped in — see [Environment variables](/configuration/environment-variables#param-fly-deploy-token). Without both, this panel shows a "Self-deploy not configured" banner and nothing else on it is actionable.
</Note>

Two KPI tiles show availability (whether a newer commit exists than the one running) and the outcome of the last self-deploy attempt.

A policy section controls what happens once a new commit is detected: deploy it automatically, or leave it to a manual **Deploy now** button, and optionally announce an available deployment to the notification webhook. Automatic deploying makes one attempt per commit — a failed automatic deploy is not retried.

Triggering a deploy, automatically or by hand, pauses new dispatches immediately and waits for in-flight work to drain before the build starts.
...
| `GET` | `/api/report` | Fleet-wide delivery metrics for a trailing window (`?days=`) and optional `?project=` filter |
...
| `GET` | `/api/parked` | List issues parked by the dispatch breaker after repeated consecutive failures |
| `POST` | `/api/parked/unpark` | Unpark an issue so it dispatches again on the next poll |

#### Deployments

| Method | Path | Description |
|---|---|---|
| `GET` | `/api/deployment-status` | Get self-deploy configuration, availability, and last deploy outcome |
| `POST` | `/api/deploy` | Trigger a self-deploy: pause dispatch, drain in-flight work, then build and release |
| `POST` | `/api/deploy-policy` | Update the auto-deploy and announce-availability policy flags |
...
These endpoints are called by **runners** (GitHub Actions workflow jobs, Fly Machine sessions, or local Docker containers running an issue), not by operators. They use a separate authentication scheme from the bearer tokens above.
...
| `POST` | `/api/runner/dependency-token` | Mints a short-lived, read-only token scoped to every repository the GitHub App installation can see, for a run whose project mapping has [`dependencyTokenScope`](/configuration/team-repo-mappings#param-dependency-token-scope) set. Authenticated with the same progress token as `/runner/progress`, despite the `/api/*`-shaped path. |

All four endpoints respond `501 Not Implemented` when the orchestrator has no `RUNNER_TOKEN_SECRET` configured, since none can validate tokens without it.
