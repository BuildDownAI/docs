...
<Steps>
  ...
  <Step title="Step through the remaining configuration">
    ...
    <Warning>
      ...
      You must also add `AWS_BEDROCK_ROLE_ARN` as a secret in the target repo. If the repo is still on the legacy `comment-trigger.yml` contract, also set the `AI_IMPLEMENT_PROVIDER` and `AI_IMPLEMENT_AWS_REGION` repo variables so comment-triggered gap-fill runs use the same auth path — a repo on the current contract inherits this from the mapping automatically.
    </Warning>
  </Step>
...
| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/github/webhook` | Receives GitHub webhook events from your target repos — pull request reviews, review comments, issue comments (including `/ai-implement` gap-fill triggers), and pull request merges. Validates the HMAC signature against `GITHUB_WEBHOOK_SECRET`. Returns `503` when the secret is unset (endpoint disabled). |
| `POST` | `/trigger/gap-fill` | Starts a gap-fill run against an existing pull request from your own automation — not used by the `/ai-implement` PR-comment trigger, which goes through `/api/github/webhook` above. Validates a bearer token equal to `GAP_FILL_TRIGGER_SECRET`. Returns `501` when the secret is unset (trigger disabled).
