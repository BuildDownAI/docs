...
<Note>
  `/api/auth` is the only *operator-facing* endpoint that does not require a bearer token — every other route documented on this page returns `401 Unauthorized` if the token is missing or expired. A separate handful of `/api/*` routes used by runner sessions authenticate differently; see [Session-machine endpoints](#session-machine-endpoints) below.
</Note>
...
Three more `/api/*` routes are called by Fly Machine and local Docker runner sessions, not by operators, and — unlike the rest of `/api/*` — do **not** require a bearer token. Each authenticates with a per-machine nonce instead.
...
<Note>
  Operators do not normally call these directly. They're documented here for the same reason as the runner callback endpoints above — so monitoring and ingress rules account for them correctly.
</Note>
