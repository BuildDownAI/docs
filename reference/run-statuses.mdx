...
What the explanation says depends on how the run ended:

| Outcome | What it tells you |
|---|---|
| `review_failed` | A pull request was opened but the automated review flagged it. Review that feedback before merging. |
| `failed` | The phase failed, naming the runner's exit code or the fact that no pull request was opened. Check the run logs, then re-dispatch. |
| `timed_out` | The run hit the time limit, and says so explicitly when a sweep ended it at the maximum session age. Likely over-scoped — split the ticket or raise the run limits. |

Each one also links to the troubleshooting guide. A run that finished cleanly gets no explanation, and neither does a sweep that tidied up an issue already marked done — there is nothing to account for.
...
| `file-overlap` | Declared files overlap sibling {issue}: {files}. Deferred until it merges. | The issue declares files a sibling on the same grouping branch has already claimed |
...
| `issue_completed_sweep` | The reconciliation sweep ended a Fly Machine session whose issue was already completed or canceled. |
