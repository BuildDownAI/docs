...
<AccordionGroup>
  ...
  <Accordion title="/ai-implement comment does nothing">
    ...
    1. **The orchestrator's GitHub webhook isn't configured.** The trigger runs through this webhook rather than a workflow file in the target repo, so it needs `GITHUB_WEBHOOK_SECRET` set on the orchestrator and the webhook active on the GitHub App — see [Configure webhook delivery](/setup/github-app). If the repo still has an old `comment-trigger.yml` file, use the **Sync workflows** action on the project's row in the admin UI to move it onto the webhook-based trigger.
    2. **The PR was not opened by AI-Implement.** The comment trigger only dispatches against PRs AI-Implement has a dispatch record for. PRs created by hand have no such record.
