...
It requeues an issue up to three times, and each attempt within the budget cancels the run and requeues the issue the same way — the second and third attempts add nothing beyond another cancel-and-requeue.

The fourth time the same issue goes overdue or fails, the budget is spent and recovery changes. The orchestrator still cancels the run and clears the in-progress marker, but it leaves the dedup entry in place, so no poll re-dispatches the issue. It records the job as `timed_out` with the `stuck_giveup` reason, posts an **AI Implementation Stuck — Needs Human** comment on the ticket naming the attempt count and the last run, and sends a notification.
