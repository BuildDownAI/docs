...
Comprehensive reference for the error codes, statuses, and failure messages AI-Implement surfaces. For step-by-step fixes to the most common problems, see [Troubleshooting](/reference/troubleshooting).
...
<Columns cols={2}>
    ...
    <Card title="Overdue run recovery" icon="arrows-rotate" href="#overdue-run-recovery">
        How the orchestrator recovers a dispatch that stops reporting back.
    </Card>
...
| Reason | Meaning |
|---|---|
| `stuck_requeued` | A run passed its time budget; the orchestrator stopped it and requeued the issue for another attempt. See [Overdue run recovery](#overdue-run-recovery). |
| `stuck_giveup` | A run passed its time budget with the retry budget already spent, so the issue was handed back to a person. See [Overdue run recovery](#overdue-run-recovery). |
| `machine_max_age_sweep` | The reconciliation sweep ended a Fly Machine session that had reached the maximum session age. |
...
A run does not hang forever when it stops reporting back. The orchestrator watches each dispatch and, on its own poll cycle, recovers one that has gone quiet.

A GitHub Actions dispatch is treated as **overdue** once it has stayed non-terminal for its configured **Job Timeout (min)** plus a short grace period. [The run hit the time limit](/reference/troubleshooting#the-run-hit-the-time-limit) covers where that timeout is set and how to raise it.

When a dispatch is overdue, the orchestrator cancels the GitHub Actions workflow run, then clears the issue's in-progress marker and dedup entry so the next poll dispatches a fresh run for the same issue.

The retry is bounded. The orchestrator keeps a per-issue attempt count that spans both overdue runs and outright failures, so a mix of the two draws down one budget.

It requeues an issue up to three times, and every attempt within the budget requeues it the same way for the next poll to pick up. An overdue attempt cancels its live run before requeueing, while a failing attempt has no live run to cancel.

The fourth time the same issue goes overdue or fails, the budget is spent and recovery changes. The orchestrator clears the in-progress marker but leaves the dedup entry in place, so no poll re-dispatches the issue. It posts an **AI Implementation Stuck — Needs Human** comment on the ticket naming the attempt count and the last run, and sends a notification.

How the run itself is recorded depends on which limit was hit. An overdue fourth attempt is cancelled, and the job is recorded as `timed_out` with the `stuck_giveup` reason. A fourth attempt that reached the budget by failing has no live run to cancel and keeps the failure reason it already reported.

A spent budget does not clear on its own. The issue stays out of the automatic queue until an operator re-applies the trigger, as [Triggers and status indicators](/reference/labels) describes.
