...
| Issue's declared files overlap an in-flight sibling's | Waits until the in-flight sibling merges |
...
<Note>
  Two siblings in the same tree that touch the same files run one at a time rather than in parallel, so the second doesn't start from a stale checkout of a file the first is still changing. Which files an issue declares comes from planning output (a work unit's `Files:` list) when planning is enabled; an issue with no declared files is never deferred by this check. Once the in-flight sibling's pull request merges, the deferred one is free to run.
</Note>
...
## Merging child pull requests into the grouping branch

Once a child pull request targets a grouping branch, AI-Implement merges it automatically as soon as it's ready — for every non-paused project with a parent/child tree, whether or not that project's **Auto Merge** setting is on.
...
<Warning>
  This never merges into your default branch. The final pull request out of the tree always waits for human review.
</Warning>

<Note>
  The project mapping's [`autoMerge`](/configuration/team-repo-mappings#param-auto-merge) field is reserved for a future per-project opt-in and currently has no effect on grouping-branch merges — they happen the same way whether it's on or off.
</Note>
