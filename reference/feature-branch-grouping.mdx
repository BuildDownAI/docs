...
Nothing changes for ordinary work. An issue outside a parent/child group behaves exactly as it always has.

## How a tree becomes branches

Each parent issue owns one grouping branch. Its children target that branch, and the structure nests up to 5 levels deep. A child nested deeper than that targets your default branch instead of its true grouping ancestor — keep trees within 5 levels if every level needs to land on its own branch.
...
|---|---|
| Issue has no children | Runs as soon as capacity allows; its pull request targets the nearest parent's grouping branch, or your default branch when it has no parent |
...
| Declared files overlap a sibling's on the same grouping branch | Waits for that sibling to merge |

That last row needs both issues to have declared what they touch. The file list comes from the [Implementation Map](/customize/planning-md#implementation-map) planning comment, or from a `Files:` section in the issue description. An issue that declares nothing never waits and never makes a sibling wait, so two issues can still collide if neither says what it is changing.
...
<Tip>
  "Finished" includes canceled. Canceling a child you've decided against releases the parent instead of stalling it.
</Tip>
...
**Closing it without merging reads as a deliberate veto, and the orchestrator respects that permanently.** It will not reopen the pull request or raise a replacement, however far ahead the grouping branch runs.

If you change your mind, open a pull request from the grouping branch yourself. The orchestrator finds it by its branch pair, waits for you to merge it, and finalizes the tree as it would have done originally.

## Merging child pull requests

Child pull requests are merged into their grouping branch as soon as they are ready. The orchestrator does this on every poll, for every project that is not paused, and there is nothing to turn on.
...
<Note>
  A run whose automated review found blockers opens its pull request as a **draft**. Because drafts are skipped, flagged work is never merged past you.
</Note>

A project mapping carries an [**Auto Merge**](/configuration/team-repo-mappings#param-auto-merge) setting that looks like it governs this. It does not — nothing reads it.
