...
<Warning>
  A finding the external tool marks as minor blocks approval exactly as a blocking one does.
</Warning>

Whether the review waits for an external tool at all is set per repository — see [Custom pipeline steps](/customize/custom-steps).
