...
<Tabs>
  ...
  <Tab title="Jira">
    ...
    <Warning>
      An issue with an open **Blocks**-type issue link is skipped, the same as Linear — the block clears once the linked issue's status reaches the Done category. This only recognizes Jira's default "Blocks" link type; if your instance renamed it, gate the issue through your JQL instead. See [A Jira issue ran despite an open blocker](/reference/troubleshooting#a-jira-issue-ran-despite-an-open-blocker) for the symptom a renamed link type produces. To stop an issue from being picked up for another reason, move it to any status other than `Ready` or `Plan Approved` — those are the only two values the poll accepts.
    </Warning>
