...
<Tip>
  The trigger runs through the orchestrator's GitHub webhook rather than a workflow file in the target repo, so it needs `GITHUB_WEBHOOK_SECRET` configured — see [Configure webhook delivery](/setup/github-app). If the repo still carries the older `comment-trigger.yml` file, use the **Sync workflows** action on its project row in the admin UI to move it onto the webhook-based trigger.
</Tip>
