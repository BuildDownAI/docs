...
<ParamField body="skillsRepo" type="string">
  ...
  Only GitHub is supported — the runner clones with a GitHub-minted token, so an SSH (`git@…`) URL or a URL on another host is rejected when you save the mapping.
...
<ParamField body="autoMerge" type="boolean" default="false">
  Reserved for a future per-project opt-in. In the current release, merging a parent issue's child pull requests into their shared grouping branch happens automatically for every non-paused project with a parent/child group, and this setting has no effect on that behavior either way — see [Feature-branch grouping](/reference/feature-branch-grouping).

  Ordinary single-issue runs are unaffected regardless of this setting.

  <Note>
    Grouping-branch merges never touch `defaultBranch`. The final pull request from the grouping branch into your default branch always waits for a human.
  </Note>
</ParamField>
