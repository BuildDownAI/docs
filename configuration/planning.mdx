...
description: "How the AI planning phase works, what structured implementation map, acceptance bar, and risks comments it posts to your ticketing system, and how to enable, disable, or configure it per project mapping."
---
...
<Steps>
  ...
  <Step title="Claude posts structured planning comments">
    Claude posts three comments on the issue, in a fixed order, each with a fixed header that the implementation workflow can locate later:

    - **Implementation Map** (`## 🗺 AI Planning: Implementation Map`) — approach, the specific files to create, modify, test, or delete, and constraints the implementer must not break. Read by the implementer.
    - **Acceptance Bar** (`## ✅ AI Planning: Acceptance Bar`) — a numbered list of falsifiable claims the reviewer can check directly against the diff. Read by the reviewer.
    - **Risks & Open Questions** (`## ⚠️ AI Planning: Risks & Open Questions`) — edge cases and unknowns found while exploring the codebase. Read by the implementer and reviewer.

    <Info>
      When the issue has dependencies or sibling stories, the Implementation Map folds in cross-story coordination notes rather than getting a comment of its own.
    </Info>
  </Step>
