...
<Tabs>
  ...
  <Tab title="Target-repo workflow secrets">
    ...
    {/* AUDIT H-4: Jira target-repo ticketing secrets removed — Jira credentials are orchestrator-side only */}
    ### Ticketing

    <ParamField body="LINEAR_API_KEY" type="string" required>
      Linear personal API key. Required when your project mapping uses Linear.

      Used by the planning workflow to fetch related issues and post structured planning comments to the Linear issue, and by the implementation workflow to update issue status and labels as the run progresses.
    </ParamField>

    <Note>
      Only Linear requires a secret on the target repo. Jira status updates are handled entirely by the orchestrator service — no Jira secrets need to be added to target repos. See the **Orchestrator runtime** tab for where `JIRA_TOKEN`, `JIRA_CLOUD_ID`, and `JIRA_SITE_URL` are configured.
    </Note>
