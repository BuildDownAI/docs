...
<Tabs>
  <Tab title="Orchestrator runtime">
    ...
    <ParamField body="GITHUB_WEBHOOK_SECRET" type="string">
      ...
      Leaving it unset disables two capabilities: **automatic follow-up runs** when a reviewer leaves blocking feedback on a pull request, and the **`/ai-implement` comment trigger** itself, which now runs through this same webhook rather than a workflow file in the target repo. Nothing polls for either, so without a working webhook neither happens.
    ...
    <AccordionGroup>
      ...
      <Accordion title="Gap-fill trigger">
        <ParamField body="GAP_FILL_TRIGGER_SECRET" type="string">
          ...
          This is not needed for the `/ai-implement` comment trigger — that path is handled by the orchestrator's GitHub webhook instead, gated by `GITHUB_WEBHOOK_SECRET`. Set this only if you drive gap-fill runs from your own automation.
        </ParamField>
      </Accordion>
      ...
      <Accordion title="Dispatch breaker (parked issues)">
        After enough consecutive dispatch failures for the same issue and phase, the orchestrator stops retrying it automatically. The issue is "parked" until an operator unparks it from the admin UI's [Runners panel](/reference/admin-ui#runners) or the `/api/parked/unpark` endpoint.

        <ParamField body="DISPATCH_BREAKER_THRESHOLD" type="number" default="3">
          Number of consecutive failed dispatches for the same issue and phase before it is parked.
        </ParamField>
      </Accordion>

      <Accordion title="Self-deploy">
        Lets the orchestrator redeploy itself on Fly.io when a newer image becomes available, surfaced in the admin UI's [Deployments panel](/reference/admin-ui#deployments).

        <ParamField body="FLY_DEPLOY_TOKEN" type="string">
          Fly API token scoped to this orchestrator's own Fly app, used to trigger the self-deploy. Distinct from `FLY_SESSIONS_TOKEN`, which only authorizes session-machine creation.
        </ParamField>

        Also requires the running image to have been built with `AI_IMPLEMENT_SOURCE_COMMIT`, `AI_IMPLEMENT_SOURCE_REPO`, and `AI_IMPLEMENT_SOURCE_BRANCH` stamped in as build arguments. These are set by the image build, not by an operator.

        Without all three build args, availability shows as unknown rather than up to date, and self-deploy stays effectively off even with `FLY_DEPLOY_TOKEN` set.
      </Accordion>
  ...
  <Tab title="Target-repo workflow secrets">
    ...
    **No ticketing provider secrets belong here.** The orchestrator now applies every Linear and Jira update itself, so the synced workflows never read a ticketing credential. 
    ...
    <Note>
      Setting these here only affects `/ai-implement` comment-triggered runs. For orchestrator-initiated runs, set per-project caps in the admin UI under **Configure → Projects**. See [Project mappings](/configuration/team-repo-mappings) for that side.
    </Note>
