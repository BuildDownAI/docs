...
<Tabs>
  <Tab title="Orchestrator runtime">
    ...
    <ParamField body="DISPATCH_BREAKER_THRESHOLD" type="string" default="3">
      How many consecutive failed dispatches for one issue and phase are tolerated before the orchestrator **parks that issue** and stops picking it up. This is the setting behind an issue that silently stops being dispatched, and a parked issue stays parked until a person clears it from the [Runners panel's **Parked issues** section](/reference/admin-ui#parked-issues).
    ...
    <ParamField body="MCP_ACCESS_TOKEN_TTL" type="string" default="3600">
      ...
      The server also issues refresh tokens, so a connected client refreshes silently rather than re-authenticating when the access token expires — this value only changes how often that background refresh happens.
    </ParamField>
