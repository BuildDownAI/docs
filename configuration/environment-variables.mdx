...
<Tabs>
  <Tab title="Orchestrator runtime">
    ...
    <AccordionGroup>
      <Accordion title="Runner callbacks (advanced)">
        ...
        <ParamField body="ORCHESTRATOR_URL" type="string">
          Public URL of the orchestrator, used in dispatch metadata. Often the same as `RUNNER_CALLBACK_BASE_URL`.
        </ParamField>
      </Accordion>

      <Accordion title="Gap-fill trigger">
        <ParamField body="GAP_FILL_TRIGGER_SECRET" type="string">
          Bearer secret that authenticates calls to the orchestrator's `/trigger/gap-fill` endpoint. When called with an issue key and PR number, the endpoint dispatches an implementation run targeting the existing PR.

          The standard synced `comment-trigger.yml` runs the pipeline directly and does not call this endpoint. Set this only if you have a custom integration that triggers gap-fill runs programmatically rather than through the GitHub comment workflow. If unset, the endpoint returns `501`.
        </ParamField>
      </Accordion>
  ...
  <Tab title="Target-repo workflow secrets">
    ...
    Target repos can be mapped to Linear, Jira, or both within a project mapping.

    <AccordionGroup>
      <Accordion title="Linear">
        <ParamField body="LINEAR_API_KEY" type="string" required>
          **Required for Linear-backed target repos.**

          Linear personal API key — used by the planning workflow to fetch related issues and post structured planning comments, and by the implementation workflow to update issue status and labels as the run progresses.
        </ParamField>
      </Accordion>

      <Accordion title="Jira">
        <Tip>
          Jira credentials are **not** configured in target-repo secrets. The implementation and planning workflows send results back to the orchestrator over a callback, and the orchestrator applies Jira status updates using its own credentials.

          Configure `JIRA_TOKEN`, `JIRA_CLOUD_ID`, and `JIRA_SITE_URL` on the orchestrator service — see **Orchestrator runtime → Ticketing → Jira** in this page.
        </Tip>
      </Accordion>
    </AccordionGroup>
    ...
    ### Bedrock — repo variables (not secrets)

    These are GitHub Actions **variables**, not secrets — configure under the **Variables** tab of **Settings → Secrets and variables → Actions**. Used so `/ai-implement` comment-triggered gap-fill runs route to the same provider as orchestrator-initiated runs.
...
| Jira API token | `JIRA_TOKEN` | `JIRA_TOKEN` | Orchestrator only |
| Jira Cloud ID | `JIRA_CLOUD_ID` | `JIRA_CLOUD_ID` | Orchestrator only |
| Jira site URL | `JIRA_SITE_URL` | `JIRA_SITE_URL` | Orchestrator only |
