...
After Claude creates a PR, the workflow reviews it and posts what it finds as PR comments. The originating issue's status and any related comments are updated separately by the orchestrator via its provider-callback mechanism.
...
See [AI planning phase](/configuration/planning) for the full flow.
...
Older repos carry a third workflow, `.github/workflows/comment-trigger.yml`, which handled `/ai-implement` comments inside the repo. Sync deletes it — the orchestrator serves that trigger now, through its GitHub webhook.

The deletion arrives in the same pull request as the workflow updates, so a repo that has not re-synced still has the file and still uses the old path.

## Security posture

The synced workflow files use two defensive practices worth knowing about:

- **Pinned action versions** — every third-party GitHub Action they call is pinned to a specific commit SHA rather than a version tag. Updates ship through the next sync rather than auto-applying when upstream releases a new tag.
...
A command-line equivalent is available if you're scripting against a self-hosted orchestrator. The workflow lives in your AI-Implement repository rather than the target repo, so dispatch it from there:

```bash
gh workflow run sync-workflow.yml --repo your-org/AI-Implement -f target_repo=your-org/your-repo
```
...
After stripping front matter and HTML comments, the workflow substitutes a fixed list of `${NAME}` variables into the body of `WORKFLOW.md` with a regular expression — not with `envsubst`, so shell semantics don't apply. Use any of them anywhere in the template.

A brace-less `$VAR` is left untouched.

<Warning>
  Any other `${UPPER_SNAKE}` token is replaced with an empty string, so a shell example containing `${...}` is silently emptied, with no error and no log line.
</Warning>
...
`PLANNING.md` is substituted the same way, and the rules above apply to it too. Its full set, including the variables describing where the issue sits in a parent/child tree:
