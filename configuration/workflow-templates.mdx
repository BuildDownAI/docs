...
Either route opens a pull request in the target repo with the updated workflow files. It seeds `WORKFLOW.md` and `PLANNING.md` only if those files don't already exist. If an earlier setup left `comment-trigger.yml` in the repo, the sync removes it — see below.
...
Older syncs installed a `comment-trigger.yml` workflow to handle `/ai-implement` PR comments. The current templates drop that file — a sync removes it if present — and route the comment through the orchestrator's own GitHub webhook instead.

<Warning>
  This makes the `/ai-implement` comment trigger depend on a configured webhook. See [Configure webhook delivery](/setup/github-app) for the setup, and [`GITHUB_WEBHOOK_SECRET`](/configuration/environment-variables#param-github-webhook-secret) for the variable it needs.
</Warning>
