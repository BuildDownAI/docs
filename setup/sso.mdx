---
title: "Configure admin sign-in with Google or Microsoft"
...
<Warning>
  Configuring both breaks MCP sign-in for anyone who can only use Microsoft: an [MCP client](/reference/mcp-tools) is never offered a choice, and always signs in with Google. Either provider works on its own.
</Warning>
...
Each variable is described in [Environment variables](/configuration/environment-variables).
...
## Add the orchestrator as a project connector

The `/mcp` endpoint can be added to [claude.ai](https://claude.ai) as a project connector, which needs one secret beyond the sign-in setup above. Without it, the connection fails at **dynamic client registration** with `invalid_redirect_uri`, before any sign-in screen appears.

The registration endpoint accepts loopback callbacks by default, so **Claude Code needs no equivalent configuration**. A browser-based client such as claude.ai calls back over HTTPS, and that origin has to be allowlisted explicitly.

<Steps>
    <Step title="Allowlist the claude.ai origins">
        Set `MCP_ALLOWED_REDIRECT_ORIGINS` as a secret with the origins claude.ai registers from:

        ```bash
        fly secrets set MCP_ALLOWED_REDIRECT_ORIGINS=https://claude.ai,https://claude.com --app <your-app>
        ```

        The callback claude.ai registers is `https://claude.ai/api/mcp/auth_callback`, and allowlisting its origin is all you do.

        <Note>
            This is not the same as the provider redirect URIs you added earlier. Those let the orchestrator finish sign-in with Google or Microsoft.

            This allowlist decides which client origins may register with the orchestrator at all, which happens before any sign-in begins.
        </Note>
    </Step>

    <Step title="Add the connector in claude.ai">
        In claude.ai, add the orchestrator's `/mcp` URL as a project connector and complete the sign-in. Registration now succeeds, and the flow continues to your configured Google or Microsoft sign-in.
    </Step>
</Steps>

Only the origins you list are accepted. Arbitrary HTTPS origins stay **denied by design**, so the allowlist is exactly the origins above unless you connect from somewhere else.

<Tip>
    A connector survives access-token expiry on its own: it refreshes silently in the background rather than sending you back through sign-in. [How long access lasts](/reference/mcp-tools#staying-signed-in) covers when someone does have to sign in again.
</Tip>
