...
Connecting a GitHub repository to AI-Implement comes down to adding a project mapping in the orchestrator's admin UI.

Saving it opens a pull request carrying the workflow templates, and starts the orchestrator polling for issues. Runs can only dispatch once that pull request is merged, so follow the steps below in order.
...
<Steps>
  ...
  <Step title="Set up the trigger in your ticketing system">
    <Tabs>
      <Tab title="Linear">
        ...
        You can create labels from **Settings → Labels** in your Linear workspace. The orchestrator makes the per-team labels it applies during a run — `AI-Working`, `AI-Planning` and `Plan-Complete` — the first time it needs them.

        <Warning>
          `Ready for Review` is the one label the orchestrator cannot create for you: it is workspace-level, and an application identity is not permitted to add one. Create it by hand before your first run — [Prepare the Ready for Review label](/setup/linear-app#prepare-the-ready-for-review-label) covers it.

          Without it, a finished run leaves the issue holding `AI-Working`, with no `Ready for Review` and no pull-request comment, so the issue looks like it is still going.
        </Warning>
      </Tab>
  ...
  <Step title="Create a project mapping in the admin UI">
    Open the admin UI at `http://localhost:8080/admin` (or your Fly.io app URL) and click **Configure → Projects → + New project**. For a first connection, the key inputs are:
    ...
    See the [Admin UI reference](/reference/admin-ui) for the full field list and defaults.
  </Step>

  <Step title="Review and merge the workflow pull request">
    Saving the project opens a pull request in the target repo that adds the following files to `.github/workflows/` and the repo root:
...
<Tip>
  One more field is available to you if you customize the pipeline. `AI-Implement Profiles` lets an issue carry names that your own [custom pipeline steps](/customize/custom-steps#profiles) branch on, and nothing built in reads it — so add it only once you have a step that wants it.

  Create it as a **Select List (multiple choices)** named exactly that, add one option per name you want to select from, and put it on the same screens as the other two.
</Tip>
...
- Review the automated review comment that Claude posts automatically, showing whether the PR is ready to merge and what (if anything) still needs attention
...
From here you can add a capability the orchestrator runs fine without, or go deeper on what you already have:
