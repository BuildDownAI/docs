...
<Steps>
  ...
  <Step title="Review and merge the workflow pull request">
    ...
    <Note>
      ...
      The sync also removes `comment-trigger.yml` if an earlier setup left it in the repo. The `/ai-implement` comment trigger now runs through the orchestrator's own GitHub webhook — see [Create a GitHub App](/setup/github-app) for the setup.
    </Note>
