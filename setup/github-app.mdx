...
<Steps>
  ...
  <Step title="Configure webhook delivery">
    Webhook delivery adds two capabilities on top of AI-Implement's poll-driven run loop: automatic follow-up on review feedback, and the `/ai-implement` comment trigger.

    **Automatic follow-up on review feedback** — when a reviewer leaves blocking comments on a pull request, AI-Implement picks them up and sends Claude back in **without** anyone commenting `/ai-implement`. An active webhook is the only route to it; there's no polling fallback.

    **The `/ai-implement` comment trigger** — once a repo is synced with the current workflow templates, it no longer carries its own `comment-trigger.yml`; the orchestrator's webhook picks up the comment instead. Without an active webhook, `/ai-implement` comments on that repo are never picked up.

    Issue pickup, run dispatch, and marking issues done on merge all stay on the poll loop and need no webhook.
    ...
    <Tabs>
      <Tab title="Without webhooks">
        ...
        <Warning>
          Without a webhook, neither automatic follow-up on review feedback nor the `/ai-implement` comment trigger works. Address PR gaps by pushing follow-up commits yourself.
        </Warning>
      </Tab>
