...
AI-Implement uses a GitHub App — not a personal access token — to authenticate workflow dispatches and to open pull requests.
...
<Steps>
  ...
  <Step title="Set repository permissions">
    ...
    <Note>
      **Pull requests** here is what lets AI-Implement open them. Every synced workflow authenticates as this App rather than as the default `github-actions[bot]` actor, so a target repo's **Allow GitHub Actions to create and approve pull requests** setting governs a different identity and has no bearing on AI-Implement's runs.
    </Note>
  </Step>
...
Each secret below notes its counterpart on the orchestrator where the names differ. See [Environment variables](/configuration/environment-variables) for the full set, including the ones that live only on the orchestrator.
...
| `ANTHROPIC_API_KEY` | Anthropic API key — used when no OAuth token is present |

Set one of the two Claude credentials, not both. Setting both does not buy you a fallback: the OAuth token wins whenever it is present, so an expired one fails the run rather than deferring to the API key.
