...
The workflow uses GitHub OIDC to assume an IAM role — there is no static AWS key path. On each run it calls `aws-actions/configure-aws-credentials` once to exchange the GitHub OIDC token for an STS session, requesting a four-hour session so that a long run does not outlive its credentials.
...
<Steps>
  <Step title="Edit the repo mapping in the admin UI">
    ...
    Save the mapping. The orchestrator will now pass `provider=bedrock` and your region to the workflow on each dispatch, including `/ai-implement` comment-triggered runs.
  </Step>
  ...
  <Step title="Add the IAM role ARN as a repository secret">
    ...
    The role must be created in the same AWS account as your Bedrock inference profiles. [IAM role configuration](#iam-role-configuration) below has the trust policy and permissions it needs.
  </Step>
  ...
  <Step title="Set the model in WORKFLOW.md (and PLANNING.md)">
    ...
    A cross-region inference-profile ARN works here too — [Model IDs](#model-ids) covers both forms and the pattern they follow.

    <Warning>
      `model:` is required for Bedrock — the IDs include date stamps and are specific to your account and region, so there is no safe default.

      Nothing validates the model against the provider. Omit it and the run substitutes an Anthropic model ID, which fails when that model is invoked rather than up front as a configuration error.
    </Warning>
  </Step>
</Steps>
