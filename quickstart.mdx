...
<Steps>
  ...
  <Step title="Review and merge the workflow pull request">
    Creating the project mapping opens a pull request in your target repo containing `claude-implement.yml`, `claude-plan.yml`, and starter `WORKFLOW.md` and `PLANNING.md` files. `/ai-implement` PR comments are handled by the orchestrator directly — see [Create a GitHub App](/setup/github-app) for the webhook it needs.
