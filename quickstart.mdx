...
<Steps>
  ...
  <Step title="Configure your environment">
    ...
    <Tip>
      Set `ADMIN_ACCESS_CODE` to a non-empty value so you can reach the admin UI at the project mapping step. It's the quickest option for a first run; [admin sign-in](/setup/sso) is the better fit once you deploy.
    </Tip>
  </Step>
  ...
  <Step title="Install dependencies and start the orchestrator">
    ...
    <Tip>
      Want to run the entire loop on your own machine? `npm run dev:local` executes each job locally instead of on GitHub Actions — the orchestrator still polls your ticketing system and opens pull requests. See [Local development](/setup/deploy#local-development) for what it needs and when to use it.

      `npm run dev:run` is the smaller one: a single task read from a file, with no ticketing system and no orchestrator involved. See the [local harness](/customize/local-harness).
    </Tip>
  </Step>
  ...
  <Step title="Create a project mapping in the admin UI">
    ...
    Click **Configure → Projects → + New project** to open the stepper. For a first run, fill ticketing config and source, then accept defaults for the rest.
...
<Columns cols={3}>
  <Card title="Deploy the orchestrator" icon="server" href="/setup/deploy">
  ...
  <Card title="Enable planning" icon="list-check" href="/configuration/planning">
    Have Claude post an implementation map, an acceptance bar, and the risks it found to your ticketing system before it implements.
  </Card>
  <Card title="Project mappings" icon="folder-gear" href="/configuration/team-repo-mappings">
    Connect additional projects to repos, and tune how each one dispatches.
  </Card>
  <Card title="Admin sign-in" icon="shield-check" href="/setup/sso">
    Per-person sign-in in place of a shared access code.
  </Card>
  <Card title="Self-deployment" icon="rocket" href="/setup/self-deployment">
    Let the orchestrator build and release its own updates.
  </Card>
  <Card title="Knowledge graph" icon="diagram-project" href="/setup/knowledge-graph">
    Project memory the BuildDown skills search before they plan.
  </Card>
</Columns>
