...
2. **Survey in both directions.** The two find different things and you need both.
   ...
   - **Search with the `Grep` and `Glob` tools, not Bash `grep`/`find`.** A Bash pattern containing `|` or `()` cannot be statically bounded by the permission layer and is declined, which blocks the `process.env` set-difference and forces a slower file-by-file fallback.
